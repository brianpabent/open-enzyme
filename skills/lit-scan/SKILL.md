...
4. **Fetch.** PubMed via `fetch_pubmed_snapshot(...)`. **East-Asian sources via `local_curl_fetch(...)` — MANDATORY, not optional.** CNKI / WanFang / ChiCTR / CQVIP / ChinaXiv / Baidu Scholar are JS-gated and bot-block hosted/browser fetch; only the firewall-whitelisted local `curl` reaches them. A scan that "references the library" but falls back to hosted fetch will hit the bot-wall and mislabel it a "language barrier" (canonical miss: 2026-07-13 chronic-tophus scan).
