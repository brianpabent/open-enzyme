...
| **UOX-1 / PHYSIOLOGY** | **Configuration-level UOX × oxygen × peroxide (§1.33)** | TBD | TBD | Tests qualified configurations at the terminal-ileal clinical-cohort substrate prior; topology can be nominated only within a controlled host comparison |
...
| LBP-2 | LBP track: establish an exact-strain genetic entry point for the organism advanced by LBP-1/current evidence | TBD | TBD | Stable transformation + reporter expression; then choose among unranked configuration tests |
...
| siRNA-2 | COMP-048 human proximal-tubule delivery-handle screen | $0 | TBD | Whether the frozen human surfaceome contains a selective proximal-tubule surface-expression/topology follow-up candidate; receptor identity, internalization, accessibility, ligand binding, and polarity validation remain downstream |
...
| TCM-2 | COMP-049 fixed-set evidence qualification (Gate-1 design; not run) | $0 | 1 | Whether five TCM-derived/formula records can preserve source tier, exact tested material, polarity, attribution, exposure, and functional gaps without producing another rank |
...
- **[Medicinal Mushroom Compound × Chokepoint Mapping (comp-014)](wiki/medicinal-mushroom-compound-mapping-computational.md)** — Invalidated prior; useful fungal-mechanism leads now live on independently sourced owner pages.
...
- **[Lactoferrin Inter-Lobe Linker Redesign (comp-034)](wiki/lactoferrin-linker-redesign-computational.md)** — Invalidated ranking; the connector remains an engineering conjecture only if WT experiments identify a reproducible linker-associated failure.
...
- **[Luminal UOX Conditional Capacity and Measurement Identifiability (comp-050)](wiki/luminal-uox-break-even-identifiability-computational.md)** — Exact method map showing why urate concentration alone cannot identify UOX removal and which qualified product, capacity, source, and boundary-fate observations are required; the biological regime remains unevaluated.
- **[Uricase Topology × Oxygen × Peroxide (comp-045)](wiki/uricase-topology-oxygen-peroxide-design-computational.md)** — **Design only:** schema-3 blocked candidate layout; no biological result, powered run count, or topology ranking.
...
- **[ABCG2 Q141K Chaperone Re-screen (comp-047)](wiki/abcg2-q141k-chaperone-rescreen-computational.md)** — **INCONCLUSIVE:** this static docking setup yields no defensible ranking; rosuvastatin is excluded and vorinostat is one marginal row, not a docking-backed wet-lab priority.
- **[Cassette Compatibility — Dual-Cassette Koji Configuration (comp-010)](wiki/cassette-compatibility-computational.md)** — **Invalidated model:** exact junction processing, trafficking, expression, and retained function require direct measurement.
- **[C. utilis Uricase Cassette Compatibility (comp-011)](wiki/c-utilis-uricase-cassette-compatibility-computational.md)** — **Invalidated model:** wild type and the patent-mutation proxy remain distinct empirical construct questions.
- **[Retired Uricase Cassette Ranking (comp-022)](wiki/uricase-cassette-ranking-computational.md)** — Invalidated composite ranking; the historical 43,200-row inventory preserves unranked construct factors for matched empirical testing.
- **[TCM Gout Compound Triage (comp-013)](wiki/tcm-gout-compound-triage-computational.md)** — Invalidated, non-runnable tombstone. The nine names survive only as an unranked historical search list; COMP-049 is a pre-run mixed-source evidence-qualification design and has not produced results.
- **[Medicinal Mushroom Compound Mapping (comp-014)](wiki/medicinal-mushroom-compound-mapping-computational.md)** — Hash-bound tombstone and research rebuild; the retired rows, ranks, and production conclusions have no current authority.
- **[DAF/CD55 SCR1-4 Truncated Protease Proxy (comp-012)](wiki/daf-cd55-scr14-truncated-computational.md)** — The LOW label is invalid because the model used pLDDT confidence as accessibility; stalk truncation remains a testable design and empirical protease risk is unresolved.
- **[DAF SCR1-4 Cassette Configuration (comp-030)](wiki/daf-cd55-scr14-cassette-ranking-computational.md)** — Invalidated ranking; direct secretion and GlaA-KEX2 remain unranked configurations for a matched expression, processing, native-fold, secretion, and function test.
- **[Uricase Protease-Site Proxy (comp-001)](wiki/uricase-protease-stability-computational.md)** — Adjacent-pair matches to three unverified legacy filters plus AlphaFold pLDDT supply a fixed-input inventory; they do not establish cleavage specificity, solvent exposure, protease survival, retained activity, or fermentation performance. The shio-koji assay remains the gate.
- **[Lactoferrin Protease Proxy (comp-005)](wiki/lactoferrin-protease-stability-computational.md)** — Invalid pLDDT-as-accessibility model; it supplies no cleavage-region priority, and untargeted fragment mapping plus retained function in §1.10 remain decisive.
- **[Retired DAF/CD55 Protease Proxy (comp-006)](wiki/daf-cd55-protease-stability-computational.md)** — The HIGH label is an invalid pLDDT-accessibility proxy and the score does not discriminate the tested sequence scopes; the Ser/Thr-rich stalk remains a testable engineering liability, not a demonstrated degradation site.
- **[Food-Associated HDAC-Directed Candidate Screen (comp-007)](wiki/food-grade-hdaci-screen-computational.md)** — **Invalidated ranking:** seven materials remain an unranked evidence inventory; direct Q141K trafficking, ABCG2-attributed urate flux, epithelial exposure, and safety decide advancement.
