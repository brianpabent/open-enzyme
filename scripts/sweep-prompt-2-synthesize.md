...
10. **Constraint-closure failures** — for any quantitative feasibility or prioritization claim, ignore the inherited conclusion once and re-derive the physical system. Check required substrates, cosubstrates, cofactors, and electron acceptors; physiological concentration relative to Km or the relevant operating constant; finite mass balance and residence time; localization / transport / physical access; coproduct and safety burden; and whether stored model inputs are actually used by the implementation. A fact appearing somewhere in the corpus is not enough: ask whether it is load-bearing in the decision model. Treat repeated agreement across pages or vendors as one inherited claim, not as independent evidence. The canonical failure is comp-019 → comp-044/045: luminal urate, UOX Km, oxygen dependence, and peroxide biology were present across the corpus, but the legacy capacity model did not use them.
