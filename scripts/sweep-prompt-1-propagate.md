...
Before propagating an active COMP-backed trigger, read its record in `logs/sweep-state.json` and its current `reviews/push-review.json` receipt. A COMP with `quarantine.json` retains its complete artifact but is deterministically blocked from execution, propagation, and synthesis without model spend; do not spread any derived claim. A COMP with `invalidation.json` is a final non-runnable tombstone and has no current model-review receipt; obey its invalidated scope and use surviving science only from the named current evidence home. `eligible_with_warning` is not clean: obey the receipt's `lane_adjudication.propagation_allowed_scope` and `forbidden_inferences`. When that scope says corrective-only, repair or retract stale claims but do not spread any derived claim. A `blocked` COMP is excluded by the coordinator. The coordinator handles cursor state and commits. Make no commit yourself.
