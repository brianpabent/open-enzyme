...
**Stale-table tiebreak (added 2026-06-01).** When a Pass 2 finding cites a load-bearing number (cost, residue, dose, "§X.Y says Z"), do NOT assert the first corpus location you find as ground truth — grep for a *second* location (a detail header vs. a downstream summary table) that might disagree. **Stale summary tables that contradict a re-scoped header are a known trap.** When two locations diverge, the **re-scoped / more-recently-updated / "updated YYYY-MM-DD"-labeled location wins**; verdict `Push back.` / `Augment.` on the *internal inconsistency* (recommend reconciling the stale location to the current one), NOT "the synthesizer's number is wrong." Canonical failure: 2026-05-30 §1.25 DAF cost — the prior reviewer cited the stale $3,500–5,500 summary table over the re-scoped $4,445–6,745 header without noticing they disagreed.
