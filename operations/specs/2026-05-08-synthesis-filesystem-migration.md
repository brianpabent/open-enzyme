...
Format (wiki daemon, amended 2026-07-15 after a same-day recovery collision): `<sweep-date>-<artifact-id>-<type>-<index>-<slug>.md`
...
- `<artifact-id>`: first eight hex characters of the normalized manifest's `sweep_id`, which is derived from the trigger commit and raw synthesis SHA-256. This makes retries/recoveries distinct even when date, type, index, and headline are identical. Direct-write producers such as `chembl-refresh` retain their documented producer-specific names.
...
Example: `synthesis/queue/2026-05-09-a68eaeb8-connection-1-rosmarinic-acid-c3-convertase-mining.md`
...
- **Q1 file naming:** originally added `<index>` for within-run collision robustness. The 2026-07-15 exact-recovery production test showed that same-day artifacts can share date/type/index/headline and overwrite one another; wiki-daemon filenames now also include the hash-derived `<artifact-id>`: `<sweep-date>-<artifact-id>-<type>-<index>-<slug>.md`.
