...
The TLR4-MD2 docking model (Yu 2026 PMC12937656) adds a proposed CP1-level mechanism separate from complement. {Model A: "entirely orthogonal to complement and CFH — represents a separate CP1 chokepoint where HCP could be effective regardless of CFH status" | Model B: does not raise this as a separate consideration}. The current [Houttuynia evidence page](../../../wiki/houttuynia-cordata.md) keeps the complement and macrophage-direction hypotheses separate and does not treat docking as functional validation.
