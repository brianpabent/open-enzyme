...
**Research track:** EPI enzyme replacement via engineered koji
...
1. **Same organism, same enzyme class.** *A. oryzae* lipase is what BoulderBio sells and what the engineered koji configuration would produce. An engineered version must outperform the OTC comparator to justify the additional complexity.
...
The documented n=1 case is a structured self-experiment on BoulderBio dose and timing. Early findings (3 weeks of daily logs, ~30 meals tracked):
...
**Implications for the engineered-koji track:**

1. **The 40,000 FIP benchmark was not enough at 1 cap/meal in this n=1 case.** A candidate *A. oryzae* product would need to deliver substantially more lipase per dose, or be tested with a higher-dose schedule. The 1,813–2,280 U/g lipase yields cited in §5.2 translate to **~50,000–60,000 FIP per dried gram** at typical assay conditions, so a 1 g engineered-koji dose could theoretically match the tested 2-cap dose.
2. **Split-dose performance suggests dose magnitude and duration both matter.** Sustained-release formulation and split dosing are testable design variables, not established product instructions.
3. **The decoupling of liquid stool from pain on the 2-cap protocol is an n=1 efficacy signal** for the enzyme-replacement mechanism before engineering. It does not establish population efficacy.
