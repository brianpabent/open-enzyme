...
>
> **Three known unknowns before CP0 can be called "closed":** (1) does the truncated construct actually fold correctly with all 12 disulfide bonds (3 per SCR domain × 4 SCRs) when expressed in *A. oryzae*? (2) does the soluble truncated fragment retain CCP-regulatory function (C3b/C4b binding + decay-accelerating activity)? (3) does luminal-side DAF actually engage submucosal-macrophage CP0 priming, or only act at the mucus interface? Each is a real wet-lab gating question; none individually fatal but cumulatively they could derail the closure thesis. Concrete next move: clone SCR1-4 (aa 35–285) with the koji-native α-amylase signal peptide, express in *A. oryzae* (RIB40 or NSlD-ΔP10 if Maruyama lab path opens), assess secretion + disulfide folding by SDS-PAGE, then functional CCP-regulatory activity in a C5a-generation inhibition assay. Estimated $2.5–4K, 6–8 weeks. The platform's CP0 coverage status has moved from "pharma-only, document the honest gap" to **"active engineering candidate with a real wet-lab plan."**
>
