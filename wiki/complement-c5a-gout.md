...
**Computational gate — comp-NNN SERPING1 protease-stability + glycosylation feasibility queued 2026-05-16.** Same comp-006/comp-012 pipeline as DAF/CD55. Cost: $0, time: ~1 week. Decides whether C1-INH is a viable LBP-luminal CP0 candidate. **If GREEN:** promote C1-INH (LBP-luminal) as the sister CP0 candidate to H05 (DAF SCR1-4, koji-secreted) and elevate this two-chassis architecture from candidate strategy to active platform thesis. **If RED:** C1-INH drops from the LBP payload candidate list and the architecture collapses back to the single-node DAF-only configuration.
