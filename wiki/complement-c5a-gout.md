...
- **Helicteres benzofuran lignans (Medium, replication-bounded):** Yin 2016 (PMID 27834928 / PMC6273495) depletion-rescue identifies compound 5 targets as C1q + C2 + C3 + C9 (CH50 9 μM / AP50 21 μM). CP > AP potency is inconsistent with CFH-competitive mechanism. Confidence bounded by [comp-018 Phase 2 INCONCLUSIVE replication of Yin 2016](./upstream-complement-modulator-sweep-computational.md).

**Two-model agreement on classification (all four candidates CFH-independent), preserved disagreement on prediction strength** (Model A predicts negative direction — carriers benefit MORE because Y402H baseline severity amplifies absolute effect size; Model B predicts null — mechanism independence implies genotype indifference). Both reject the AMD-paradox direction (carriers worse). UKB cross-tab specification pre-specifies both with separate falsification thresholds; a positive (carriers worse) result refutes both and forces retiring the upstream-CP0-bypass hypothesis. Total OpenRouter spend for comp-039: $0.0022 (Model B counter-reads only; Claude subagent was Model A natively per `memory/feedback_subagent_as_model_a.md`).
...
**Comp-039 (2026-05-21) per-candidate cross-tab specification.** The candidate-stratified UKB ask (specific, falsifiable, mechanism-grounded) replaces a generic "any-polyphenol × CFH" query:
...
**CFH Y402H stratification (added 2026-05-22, propagation from `gout-genetic-variants.md` Category 5 + `cfh-mechanism-dissociation-cp0-candidates-computational.md`).** The dietary CP0 candidates referenced above (rosmarinic acid, luteolin, Houttuynia cordata polysaccharide, Helicteres benzofuran lignans) are predicted to be **CFH-independent** in their mechanism of action — they operate upstream of Factor H regulation rather than downstream of it. This creates a genotype-stratified prediction: carriers of the CFH Y402H variant (rs1061170 risk allele; ~36-39% European / ~35-37% African / ~30% South Asian / ~5-6% East Asian) have a weakened Factor H protein that under-regulates alternative-pathway C3b deposition on MSU crystal surfaces. If the dietary CP0 candidates work by reducing fluid-phase C3 convertase substrate (the rosmarinic acid mechanism per comp-029) or by blocking complement entry at C2/C4/C5 (the Houttuynia polysaccharide mechanism per comp-018), Y402H carriers may benefit **more** from these candidates than wild-type carriers, because they have more unregulated C3b deposition to suppress in the first place. Counter-evidence flag: the parallel hypothesis in age-related macular degeneration (AMD) inverted in the AREDS trial — Y402H high-risk carriers did *paradoxically worse* on the zinc/antioxidant formulation (Vavvas 2018; Merle 2015 found DHA protective only in non-risk genotypes; Klein 2008). The OE hypothesis is that AMD interventions work *through* Factor H (which carriers can't capitalize on) while these dietary candidates work *upstream of* Factor H (which carriers can capitalize on). That is hand-wavy until empirically tested. **Biobank cross-tab path:** UK Biobank and All of Us as co-primary cohorts (per `cfh-mechanism-dissociation-cp0-candidates-computational.md` §5; AoU elevation 2026-05-22 reflects CFH 402H allele-frequency parity between European and African ancestries + Volcik 2008 ARIC effect-direction heterogeneity that makes single-cohort cross-tabs insufficient for cross-ancestry generalization). The lead query is rs1061170 × Phenol-Explorer-derived rosmarinic-acid intake quartiles × incident gout M10.x. **Operational consequence for §9.7's combined strategy:** if the CFH-stratification prediction holds, the rosmarinic-acid arm of §9.7's composition has differential expected effect-size by patient genotype — a precision-medicine angle on top of the mechanism-coverage claim. The engineered DAF SCR1-4 arm is also predicted to be CFH-independent (DAF acts at the convertase-decay step downstream of Factor H), so the §9.7 composition is CFH-independent at both arms — but the *dietary* arm is the one with the dietary-intake variable that biobank cross-tabs can score. **The CFH stratification adds an interpretive layer to the §9.7 combined-strategy outcomes; it does not change the recommended interventions or the §1.25 wet-lab gate.**

**Cross-references:** [`upstream-complement-modulator-sweep-computational.md`](./upstream-complement-modulator-sweep-computational.md) (comp-018 dietary scan), [`upstream-complement-verification-rerun-computational.md`](./upstream-complement-verification-rerun-computational.md) (comp-020 verification re-run; the 44× IC50 spread caveat lives here), [`daf-cd55-scr14-truncated-computational.md`](./daf-cd55-scr14-truncated-computational.md) (comp-012 engineering feasibility), [`chaperone-orthogonal-stacking.md`](./chaperone-orthogonal-stacking.md) §3.5.3 (effective PDI load), [`medicinal-mushroom-extract-sops.md`](./medicinal-mushroom-extract-sops.md) (Tier 2 readout for rosmarinic acid potency in dietary sources), [H05 — DAF SCR1-4 CP0 thesis](./hypotheses/H05-daf-scr14-cp0-thesis.md), [`cfh-mechanism-dissociation-cp0-candidates-computational.md`](./cfh-mechanism-dissociation-cp0-candidates-computational.md) (CFH Y402H stratification analysis + UKB ↔ AoU co-primary cross-tab specification), [`gout-genetic-variants.md`](./gout-genetic-variants.md) Category 5 (CFH Y402H variant background, allele frequencies, AMD counter-evidence).
...
- **C1-INH (SERPING1, UniProt P05155) via engineered LBP.** [comp-024](./complestatin-bgc-lbp-feasibility-computational.md) returned RED for complestatin and **GREEN-provisional 0.774** for C1-INH-on-EcN in the LBP-luminal chassis. C1-INH inactivates C1r/C1s and MASP-2 at the classical/lectin pathway **entry point**, preventing C4b2a convertase formation upstream.
- **DAF SCR1-4 (aa 35–285, UniProt P08174) via engineered koji.** [comp-012](./daf-cd55-scr14-truncated-computational.md) confirmed feasibility; [H05](./hypotheses/H05-daf-scr14-cp0-thesis.md) formalizes the thesis; [§1.25](./validation-experiments.md) is the wet-lab gate. DAF accelerates decay of **already-assembled** C4b2a and C3bBb convertases at the MSU crystal surface.
...
> **⚠ Gut-compartment co-localization caveat (added 2026-06-01, per the RA PK scan in [`open-questions.md` §"Riskiest assumption #3"](./open-questions.md)).** This composition assumes both arms act at the same gut site, but the RA PK scan suggests they may **not co-localize.** Engineered **C1-INH on EcN-LBP colonizes the distal gut/colon**, whereas **intact rosmarinic acid is degraded by colonic microbiota** (to caffeic acid + phenylpropionics) *before* reaching the distal gut — RA's intact-form CP0 activity is most defensible **proximally** (small intestine), where EcN does not predominantly reside. comp-020 verified C3b modification by *intact* RA specifically; the distal-gut species is its metabolites, whose complement activity is unverified. The two arms may therefore protect *different* intestinal segments rather than layering at one site. This does not kill the composition (segment-distributed coverage is still coverage), but it sharpens reactivation condition #2 below: the RA-arm de-risk needs **segmental** gut-luminal PK (where is *intact* RA present?), not just aggregate "adequate tissue exposure."
...
**CP0 intervention surface bracketing (added 2026-05-22, per sweep 2026-05-21 Connection #4 + Priority Action #2).** This dormant composition sits at the **blockade-class extreme** of the CP0 intervention surface — both arms (C1-INH at classical/lectin entry; rosmarinic acid at C3 convertase) act *before* complement activation produces downstream consequences, preventing the cascade from running rather than suppressing its outputs. The opposite end of the same surface is the **suppression-class** approach documented in [`cannabinoids-terpenes.md` §"Brian's n=1 observation"](./cannabinoids-terpenes.md) — the four-route layered flare-interrupt (prednisone + topical CBD:THC + inhaled cannabis + ice) acts entirely downstream of complement activation through independent receptor pathways (GR, CB2, TRPV1 desensitization, neutrophil chemotaxis). The two compositions bracket the CP0 design space and clarify what the platform's CP0 strategy *currently doesn't have*: zero active resolution-class CP0 coverage. Protocols A/B/C in [`gout-action-guide.md`](./gout-action-guide.md) are all suppression-class; the §9.9 dormant composition would be the first active blockade-class option when both component arms clear individual validation. Making the bracketing visible sharpens the CP0 research agenda — the gap is not "do we have CP0 coverage at all" (yes, via suppression) but "do we have *causal-prevention* CP0 coverage" (no, gated on the dormant arms).

**Cross-references:** §9.7 (rosmarinic acid + DAF SCR1-4); §9.8 (C1-INH + DAF SCR1-4); [`cannabinoids-terpenes.md` §"Brian's n=1 observation"](./cannabinoids-terpenes.md) (suppression-class CP0 bracket counterpart to this blockade-class composition); [`c1-inh-protease-stability-ecn-computational.md`](./c1-inh-protease-stability-ecn-computational.md) (comp-037 C1-INH gate); [`upstream-complement-modulator-sweep-computational.md`](./upstream-complement-modulator-sweep-computational.md) (comp-018 rosmarinic acid screen); [`upstream-complement-verification-rerun-computational.md`](./upstream-complement-verification-rerun-computational.md) (comp-020 rerun); rosmarinic-acid gut-luminal PK reframe per comp-029.
