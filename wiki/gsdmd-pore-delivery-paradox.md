...
5. **First gout-relevant compound to test through pores:** Ac-FLTD-CMK is the most structurally interesting candidate (GSDMD-derived, selectively targets the same protein that created the pore). A cell culture experiment in LPS+MSU-stimulated primary human macrophages comparing Ac-FLTD-CMK efficacy before and after GSDMD pore formation would directly test the paradox in the gout context. This is a $2,000–5,000 experiment if a wet-lab partner has the MSU stimulation setup.
   ...
   **Tier-1 delivery-readout precursor (added 2026-06-01, 2026-05-30 sweep Most Curious Thread).** Sequence a cheaper *delivery* test before the *efficacy* test above (one-experiment-one-decision): does a membrane-impermeant caspase/GSDMD inhibitor actually **enter** pore-forming macrophages faster? Protocol — THP-1 (or primary) macrophages + MSU stimulation, **fluorescently-labeled** membrane-impermeant inhibitor (e.g., FAM-tagged Z-YVAD-FMK or Ac-FLTD-CMK), ± controlled GSDMD-pore induction (low-dose nigericin as the pore-on lever), readout **intracellular fluorescence by flow cytometry or confocal**. This isolates the *delivery* claim (drug gets in through pores) from the downstream *efficacy* claim (#5's IL-1β/pyroptosis readout), and is the cheapest discriminator of the whole paradox. **Refutation criterion:** if membrane-impermeant inhibitor uptake is NOT higher in pore-forming vs. intact-membrane macrophages (or pore lifetime is too short for meaningful influx), the self-delivery thesis fails and #5's efficacy experiment is moot. Gate the efficacy experiment on a positive uptake signal here. **→ Now canonical as [validation §1.32](./validation-experiments.md)** (added 2026-07-13), which sharpens this precursor with comp-042's transporter-orphan reframe: the primary probe is a **transporter-orphan tracer** (no import route of its own, so the pore is the only way in), plus a PepT1-confounder control arm (fluorescent-KPV ± PepT1 inhibitor) and GREEN/RED criteria. Use §1.32 as the design of record; this note is the earlier framing.
