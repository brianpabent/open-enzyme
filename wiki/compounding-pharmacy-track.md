...
- **Small molecules** → potentially compoundable, with sub-cleavage by regulatory status:
  - Off-patent FDA-approved drugs with bulk API on the FDA 503A list → **first-class compounding-pharmacy targets** (the MINX category).
  - Off-patent FDA-approved drugs being repurposed at off-label doses or in novel combinations → **first-class compounding-pharmacy targets**.
  ...
  - Research compounds without bulk API source (MCC950, dapansutrile in some markets) → **not compoundable** until a bulk supplier is on the FDA list.
  - Peptides (KPV, BPC-157) → compoundable but more constrained (503A only for many; FDA bulk-list status varies by peptide).
...
### Routed to the compounding-pharmacy track (candidate list, evidence-tier honest)

For each entry: chokepoint mapping, regulatory status of API, evidence level for the gout / NLRP3 application, what a compounding formulation would actually look like.

**1. Disulfiram (Antabuse) — CP6b GSDMD inhibitor.**
- Regulatory status: FDA-approved 1951 for alcohol-use disorder. Off-patent. Bulk API available.
...
**2. Colchicine — CP6 prophylaxis.**
- Regulatory status: FDA-approved. Bulk API available. The Colcrys (FDA-approved formulation) and unbranded generic both exist.
...
- Regulatory status: FDA-approved for hairy-cell leukemia (IV). Off-patent. Bulk API status: limited — primarily a hospital pharmacy product, not commonly on 503A lists. Would need supplier-side verification.
...
- Compounding play: highly conditional on pentostatin oral bioavailability (currently IV-only) and bulk API availability. Probably **not first-line** compared to whole-fermentate Cordyceps from the medicinal-mushroom-complement track.
- Evidence level: **Mechanistic extrapolation** with significant pharmacokinetic unknowns.

**4. Allopurinol — XO inhibition (custom doses + combinations).**
- Regulatory status: FDA-approved 1966. Off-patent. Bulk API widely available on FDA 503A list.
...
**5. Probenecid — URAT1 inhibitor (custom doses + combinations).**
- Regulatory status: FDA-approved 1951. Off-patent. Bulk API widely available.
...
**6. Lesinurad — URAT1 inhibitor (withdrawn but mechanism-relevant).**
- Regulatory status: FDA-approved 2015 for combination with XOI; **commercially withdrawn 2019** for business reasons (not safety). Bulk API may be available depending on supplier; not on standard 503A lists.
- Gout-relevant evidence: **Clinical Trial** — efficacy established before withdrawal.
- Compounding play: depends on bulk API availability. Could be revived as a compounded formulation if a 503B outsourcing facility sourced it, but **regulatory risk is non-trivial** (withdrawn drugs face higher 503A scrutiny). Probably defer to probenecid as the URAT1 stand-in until lesinurad bulk sourcing is verified.
- Evidence level: **Clinical Trial** (for the withdrawn product); regulatory status complicates path.

**7. Zileuton — CP6a 5-LOX inhibitor.**
- Regulatory status: FDA-approved 1996 for asthma. Off-patent. Bulk API availability: uncertain, would need verification.
...
- Compounding play: if bulk API is available, **low-dose ER zileuton for flare prophylaxis** is a clean repurposing experiment. Risk: zileuton has known hepatotoxicity signal in asthma cohorts; compounded use needs liver monitoring.
...
1. **Bulk API audit.** Which of the candidate compounds above are actually on the FDA 503A and 503B bulk drug substances lists? (Maintained by FDA, updated periodically; status changes.) The list is the gating empirical question for every entry in §"Routed to the compounding-pharmacy track" above. **Tracked at:** `synthesis/queue/2026-05-13-experiment-2-execute-the-compounding-pharmacy-bulk-api-audit.md` (Item 11 of the current walkthrough). **Status:** queued; will be actioned in this walk.
2. **Pharmacy partner identification.** What 503A compounding pharmacies have track record with gout / inflammation / off-label dose work? Empower Pharmacy, Olympia Pharmacy, others — needs primary-source verification. **Status:** user-action-required (real-world outreach, not Claude-actionable). Stays on this scope page; not in the synthesis queue.
