...
- 503A eligibility: **regulatory edge case.** FDA-approved 2015 for combination with XOI; **commercially withdrawn 2019** for business reasons (not safety). Tier 2 eligibility (component of FDA-approved drug) may or may not survive market withdrawal — this is the only OE candidate where 503A eligibility is structurally uncertain rather than supply-chain-uncertain. Worth real verification via state-board-of-pharmacy precedent search before any prescription pathway. Bulk API supply also uncertain (no commercial product → likely no compounding-supplier stocking).
- Gout-relevant evidence: **Clinical Trial** — efficacy established before withdrawal.
- Compounding play: depends on resolving the Tier-2-survives-withdrawal question AND bulk API sourcing. Could be revived as a compounded formulation if a 503B outsourcing facility sourced it. Probably defer to probenecid as the URAT1 stand-in until lesinurad's regulatory status is resolved.
