...
1. **Bulk API audit.** Which of the candidate compounds above are actually on the FDA 503A and 503B bulk drug substances lists? (Maintained by FDA, updated periodically; status changes.) The list is the gating empirical question for every entry in §"Routed to the compounding-pharmacy track" above.
...
3. **Physician partner pathway.** Off-label prescribing is the physician's discretion but needs a real prescriber. Rheumatology / functional medicine cross-section. The team-building work overlaps with [`team.md`](./team.md) — possibly a fourth collaborator role.
