---
title: Koji Home Fermentation — Wild-Type Protocol for Digestive Enzyme Delivery
date: 2026-04-25
tags: [koji, fermentation, EPI, digestive-enzymes, protocol, home-scale]
related:
  - engineered-koji-protocol.md
  - gut-lumen-sink.md
  - supplements-stack.md
sources:
  - Hishiroku Chohaku-kin product instructions (Japanese)
  - Standard Japanese koji craft literature (sake/miso/shochu traditions)
  - Pubmed: Aspergillus oryzae enzyme profiles (multiple)
---
...
This is **distinct from** the project's engineered-strain work in [engineered-koji-protocol.md](./engineered-koji-protocol.md) — this page is the wild-type / pre-engineering anchor: what's possible with off-the-shelf koji-kin and home equipment, and what a project-scale engineered version needs to outperform to justify itself.
...
2. **Steam rice** 45–60 min until *al dente* — grains should be cooked through but firm, not mushy. (Boiled rice is too wet and will rot instead of growing koji.)
3. **Cool to ~32–33°C** by spreading on a clean tray and turning. Aim for the rice to feel just barely warm.
4. **Inoculate.** Sprinkle koji-kin spores evenly across the rice (a small pinch per kg of rice — Hishiroku's instructions say 20 g inoculum / 15 kg rice, so for 1 kg rice you need about 1.3 g — start with ½ tsp). Mix thoroughly with clean hands.
5. **Wrap loosely in damp cloth** and place in incubation chamber at 32–33°C.
6. **Turning schedule** (this is where the Hishiroku bag's 第1〜3回手入れ instructions come in):
...
7. **Done at 42–48 h total.** Rice grains should be visibly fuzzy white, not separated, distinctly sweet-smelling. Should NOT smell of ammonia, alcohol, or be greenish (= contamination, discard).
8. **Refrigerate immediately** to halt activity. Use within 1–2 weeks, or freeze for up to 6 months.
...
1. **Is shio-koji-marinated protein a meaningful PERT-reducer in mild-to-moderate EPI?** N=1 / household trials with PERT-dose-per-meal tracking would generate informative observational data.
2. **What's the practical enzyme-activity ceiling of wild-type koji-fermented foods?** Measurable in lab (amylase / protease assays of finished shio-koji) vs. commercial PERT activity per dose.
3. **What's the palatability + adherence ceiling?** A homemade enzyme strategy only works if the user actually eats the resulting food regularly.
...
A pedagogical note for the engineered-strain rollout: the engineered-strain platform's distribution model — a frozen / lyophilized master spore stock that users return to for each new starter, with no backslopping past a small generation count — maps directly onto the tane-koji → koji rice pattern documented above. The "buy tane-koji once per ~15 kg of rice" rule already is the limited-generation propagation rule that industrial fermentation enforces with frozen master cell banks. See [open-source-platform.md §"Cultural mapping: tane-koji as master, koji rice as working batch"](./etc/open-source-platform.md#cultural-mapping-tane-koji-as-master-koji-rice-as-working-batch) for how the platform's Strain Stability Kit lines up against this convention.
