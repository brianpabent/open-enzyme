...
**BioDesignBench empirical support (2026-05-13, PRIMARY-SOURCE-PENDING):** A bioRxiv preprint (2026.05.06.723381) benchmarking LLM agents on 76 protein-design tasks reports that DeepSeek V3 significantly outperforms GPT-5 and Gemini 2.5 Pro on the benchmark (per abstract; full PDF verification pending). OE's sweep daemon uses DeepSeek V4-Pro (V3's successor) as the Pass 4 cross-vendor peer-reviewer. If the abstract claim holds under full-text verification, the V4-Pro anchor is not only the heterogeneity guard but also the empirically strongest protein-design reasoner among major frontier models — a substantive upgrade to the architectural justification. The paper also introduces a tool-gap vs. science-gap decomposition as a diagnostic framework for agent failures, which is a methodological-upgrade candidate for Pass 3/4 disagreement attribution. See [bio-ai-tools.md §"BioDesignBench"](./etc/bio-ai-tools.md) for full details and verification gate. (source: bio-ai-tools.md; Mechanistic Extrapolation pending primary-source verification)

**Empirical validation.** The cross-vendor heterogeneity guard has produced two documented catches: the DAF SCR1-4 disulfide-count incident (2026-05-06, caught at the pre-commit grep-verify gate — see [`etc/manual-literature-mining.md` §"Pre-commit verification gate"](./etc/manual-literature-mining.md#pre-commit-verification-gate-the-rule-that-catches-errors-before-the-sweep-not-after)) and the Paperclip Km misreport (2026-05-13, caught by DeepSeek V4-Pro's Pass 4 review nine days after entering the corpus and propagating into an external paper draft — see [`etc/paperclip-deep-dive.md` §"2026-05-13 correction"](./etc/paperclip-deep-dive.md)). The two incidents are complementary: DAF is the canonical case for catching errors at the wiki-authoring step before they propagate; Paperclip is the canonical case for catching errors that survived single-model generation and review. Full write-up of both incidents: [`papers/cross-vendor-heterogeneity-guard/draft.md`](../papers/cross-vendor-heterogeneity-guard/draft.md) (in progress).
