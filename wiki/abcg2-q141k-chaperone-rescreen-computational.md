...
2. **Axis 2 (empirical ChEMBL) — expanded, and a substrate blind spot found.** At import only 3 molecules were queried; the three surviving docking candidates (rosuvastatin, vorinostat, avacopan) were then queried against ChEMBL ABCG2 (CHEMBL5393): **all three returned 0 ABCG2 inhibition records.** This surfaced a structural limitation: **ChEMBL logs inhibition assays (IC50/Ki), not substrate transport.** rosuvastatin — one of the two "uncertain" survivors — is the canonical ABCG2 **substrate** (Q141K raises its plasma AUC ~1.6–2×; the ABCG2 statin-pharmacogenomics case in the FDA label), yet returns 0 records because it is transported by, not an inhibitor of, ABCG2. On domain-knowledge grounds rosuvastatin is **disqualified** as a chaperone candidate (a molecule that is itself an ABCG2 substrate would compete with urate for efflux), leaving **vorinostat as the sole marginal "uncertain" hit** — margin +0.55, within Vina noise. Consequence for the axis: ChEMBL-based Axis 2 validates known-*inhibitor* disqualification (elacridar pChEMBL 6.39, novobiocin 4.6 confirmed) but **cannot, by construction, catch substrate-type disqualifiers** — that needs a transporter-substrate source (UCSF-FDA TransPortal / PharmGKB / DMET), not ChEMBL. Recorded in [`outputs/chembl_axis2.json`](./etc/experiments/comp-047-abcg2-q141k-chaperone-rescreen/outputs/chembl_axis2.json).
