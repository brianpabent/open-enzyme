...
**Companion intervention: PDB-engineered EcN (urate-disposal upstream of NLRP3):** Disulfiram closes the pyroptotic exit *downstream* of NLRP3, but does nothing to reduce the urate substrate that triggers NLRP3 in the first place. The natural companion is engineered *E. coli* Nissle expressing the purine-degrading-bacteria (PDB) gene cluster (CBT2.0 precedent, Li 2025, −63% plasma UA in mice) — drains luminal urate and produces butyrate → ABCG2 induction + NLRP3 dampening *before* MSU crystals seed an inflammasome. The two arms hit opposite ends of the urate→inflammation axis on completely separate chassis (engineered LBP vs. compounded small molecule), share no production or formulation stack, and stack additively. See [`purine-degrading-bacteria.md`](./purine-degrading-bacteria.md) for the PDB mechanism and [`chassis-pending-interventions.md` §"Multi-chassis stacks" M1](./chassis-pending-interventions.md) for the dual-arm framing. **Computational priors completed 2026-05-16:** [comp-027](./disulfiram-dose-modeling-computational.md) (disulfiram dose window → YELLOW-leaning-GREEN, 100 mg/day sub-AUD) + [comp-031](./dual-chassis-ecn-pdb-uricase-computational.md) (dual-chassis EcN additive SUA → YELLOW provisional, ~25–30% larger than PDB-alone). (source: disulfiram-dose-modeling-computational.md, dual-chassis-ecn-pdb-uricase-computational.md) **PK-interaction check (2026-07-13):** the proposed butyrate × disulfiram-CYP interaction was resolved **PK-clean** — no material effect, no dosing caveat (disulfiram is a CYP2E1 inhibitor not a substrate; gut butyrate is exposure-limited to ~2–3 orders below hepatic-CYP-effective concentrations; GSDMD blockade is covalent/nanomolar). Full analysis: [`chassis-pending-interventions.md` §M1](./chassis-pending-interventions.md).
...
- **Gout prophylaxis (sub-AUD GSDMD-dominant window):** **100 mg/day (range 75–125 mg/day)** — per [comp-027 dose modeling](./disulfiram-dose-modeling-computational.md) (2026-05-16, YELLOW-leaning-GREEN). At 100 mg/day, parent DSF Cmax (0.40 µM) engages GSDMD pore-formation blockade at 1.3× cell-free IC50 (0.30 µM per Hu 2020) while plasma Me-DTC peak (~70 nM) stays at or below the Faiman-1989 DER hypotension threshold (~70 nM at 40% ALDH inhibition). NLRP3-palmitoylation EC50 (Xu 2024, 10 µM) requires AUD-dose+ plasma — sub-AUD is selectively GSDMD-engaging, not pan-NLRP3-engaging. Two-phase compounding protocol: IR capsule 50→100 mg/d titration over 14 days, then ER lipid-matrix 100 mg QD chronic. (Mechanistic Extrapolation + In Silico; source: disulfiram-dose-modeling-computational.md)
