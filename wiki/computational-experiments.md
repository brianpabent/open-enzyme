...
| comp-022 | ClockBase-style exhaustive combinatorial ranking of *A. oryzae* uricase expression cassettes (in silico): promoter × signal peptide × codon variant × secretion scaffold (~43,200 combos); score each candidate with AlphaFold pLDDT, chaperone-load per [`chaperone-orthogonal-stacking.md`](./chaperone-orthogonal-stacking.md), codon adaptation index, and mRNA secondary structure; promote to wet-lab via N-of-M concordance threshold (≥4 of 5 models, with the exact threshold calibrated against retrospective comp-001 through comp-014 cassette-design outcomes per the open methodology question in [`autonomous-screening-methodology.md`](./autonomous-screening-methodology.md)). Instantiates the ClockBase hypothesis-then-verify pattern at run scope. Verification agent pass per [§Infrastructure proposals](#comp-nnn-verification-agent-clockbase-hypothesis-then-verify-pattern--added-2026-05-08) before commit. | [`validation-experiments.md`](./validation-experiments.md) §1.9 dual-cassette feasibility gate; replaces literature-precedent one-at-a-time cassette selection with composite-ranked shortlist before gene synthesis dollars commit; instantiates ClockBase methodology documented in [`autonomous-screening-methodology.md`](./autonomous-screening-methodology.md) | High (next free in silico run; gates §1.9 wet-lab spend) |
...
| comp-023 | Metabolic / proteome burden of cns1+cns2 cordycepin biosynthesis on top of the dual uricase + lactoferrin cassette in *A. oryzae*, in silico: flux-balance analysis or pcSecKoji-style proteome-constrained metabolic model evaluating ATP, SAM, precursor (adenosine, GTP-derived adenine pool), and translation-pool burden. Comparators: (a) wild-type baseline, (b) dual uricase + lactoferrin cassette only, (c) existing optional cytosolic third cassettes (carnosine via carnS, panD) scored in [`chaperone-orthogonal-stacking.md`](./chaperone-orthogonal-stacking.md) to give a relative-burden ranking. Closes the proteome-burden gap that the chaperone framework explicitly flags as out-of-scope (line 238). Verification agent pass before commit. **Decision thresholds:** predicted growth penalty <10% AND native kojic acid + EGT yields >80% of dual-cassette baseline = green-light a cordycepin arm in §1.9 extended design; >10% growth penalty OR >20% native yield reduction = red-flag, route cordycepin to a separate strain or to the LBP chassis peer track instead. | [`chaperone-orthogonal-stacking.md`](./chaperone-orthogonal-stacking.md) (closes the framework's named proteome-burden gap); [`medicinal-mushroom-complement-track.md`](./medicinal-mushroom-complement-track.md) (Jeennor 2023 *A. oryzae* cordycepin at 564 mg/L/day precedent); [`koji-endgame-strain.md`](./koji-endgame-strain.md) (extended §1.9 design depends on the green/red verdict). Decides whether the endgame strain acquires a third therapeutic chokepoint (URAT1 modulation via cordycepin) on top of urate degradation + NLRP3 suppression. | High (gates the cordycepin arm of the §1.9 extended design; cheapest discriminating in silico test before any wet-lab co-expression commit) |
