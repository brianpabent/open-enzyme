...
**No computational prior for lactoferrin.** Lactoferrin's 80 kDa glycoprotein structure (two bilobal domains, exposed N/C-terminal linker) is structurally distinct from uricase's compact homotetramer — comp-001's verdict does not extend to it. The **lactoferrin arm of §1.10 remains a feasibility gate.** If wet-lab shows lactoferrin degradation while uricase survives, the shio-koji format is viable for uricase but the dual-payload endgame strain would need a different delivery format for its lactoferrin cassette.
