...
> **#1 priority gate (elevated 2026-04-27, re-scoped 2026-05-16 per chaperone-framework calibration verdict):** the platform-architecture gating experiment is the **chaperone-orthogonal stacking framework's α-coefficient calibration set** — [§1.9 Ward 1995 dual-cassette feasibility test](#19-ward-1995-dual-cassette-feasibility-test-koji-endgame-strain-gate-1-priority-gate) **paired with** [§1.25 DAF SCR1-4 single-cassette expression](#125-dafcd55-scr1-4-truncated-single-cassette-expression-in-a-oryzae-cp0-engineering-candidate-wet-lab-gate). Either is informative alone, but the framework calibration only works as a pair under harmonized conditions (matching host = NSlD-ΔP10, matching format = solid-state shio-koji, matching promoter, matching titer units). The α coefficients drive whether DAF SCR1-4 routes to a separate strain vs the endgame strain, whether triple-cassette designs are feasible, and whether the single-strain endgame thesis holds — see [`chaperone-orthogonal-stacking.md`](./chaperone-orthogonal-stacking.md) §3.5.4 + §8 item 6 for the framework's own calibration uncertainty. Until this calibration set returns empirical data, every framework-driven architecture decision rests on α coefficients derived from non-koji in vitro folding kinetics. Combined cost ~$10–15K if run independently (§1.9 $5,265–8,065 + §1.25 $4,445–6,745, both updated 2026-05-17), less under co-batch execution (shared *A. oryzae* protoplast-transformation + solid-state fermentation infrastructure per §1.25 Dependencies); ~8–12 weeks wall-time if run in parallel. §1.9 also independently gates the koji-endgame single-strain thesis (its original #1 role); §1.25 also independently gates the DAF SCR1-4 CP0 engineering candidate (per [H05](./hypotheses/H05-daf-scr14-cp0-thesis.md)).
...
| [§1.9](#19-ward-1995-dual-cassette-feasibility-test-koji-endgame-strain-gate-1-priority-gate) | **Ward 1995 dual-cassette feasibility (koji endgame gate)** — **#1 priority** | In Vitro | $5,265–8,065 | 8–12 | Proposed | [koji-endgame-strain](./koji-endgame-strain.md), [lactoferrin](./lactoferrin.md), [engineered-koji-protocol](./engineered-koji-protocol.md), [aspergillus-oryzae](./aspergillus-oryzae.md) |
...
**Cross-references:** [synthesis/](../synthesis/README.md) 2026-04-24 Connection #4 + Proposed Experiment #2; [lactoferrin.md](./lactoferrin.md) §3 (LPS/CD14 sequestration mechanism); [egcg.md](./egcg.md) (20S proteasome 86 nM target, hepatotoxicity ceiling); [nlrp3-exploit-map.md](./nlrp3-exploit-map.md) v1.2 CP1a (independent input/output barrier framing); [supplements-stack.md](./supplements-stack.md) (current standalone entries for both compounds); [koji-endgame-strain.md](./koji-endgame-strain.md) (downstream engineering implication, gated on §1.9).
