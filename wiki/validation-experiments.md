...
**Computational boundary (comp-022 v2, corrective review open):** The artifact enumerated 43,200 promoter × signal-peptide × codon × scaffold combinations. Its current shortlist, N-of-5 tiers, direct-secretion cluster, gene-synthesis refinements, and GlaA-KEX2 ordering are not decision-eligible because one ranking axis uses uncalibrated chaperone-load coefficients and the axes were not jointly calibrated to a named biological outcome. Treat the enumerated parts as a hypothesis inventory only. Select exact configurations through a fresh reviewed design, then measure expression, processing, localization, native fold, function, oxygen/peroxide behavior, and viability. See the [current evidence boundary](./uricase-cassette-ranking-computational.md) and [open actions](../synthesis/queue/comp-review-022.md).
