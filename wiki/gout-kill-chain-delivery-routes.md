...
- **Purine-degrading bacteria (PDB) — 2025 discovery:** A 2025 bioRxiv paper (April 2025) identified a conserved gene cluster in human gut Bacillota (Lachnospiraceae and related) that converts urate to SCFAs (lactate, butyrate) via a 2,8-dioxopurine pathway. Antibiotic depletion of PDB taxa elevates serum urate in healthy volunteers. Probiotic supplementation with defined PDB strains is a Phase 0 oral modality for urate lowering via a completely distinct mechanism from uricase enzyme replacement. The OE wiki does not yet cover this. (Animal Model + Human Observational — antibiotic depletion; Mechanistic Extrapolation — probiotic supplementation)
...
### 5. Purine-degrading bacteria (PDB) — missing from the current OE wiki

April 2025 bioRxiv paper identifies conserved *Bacillota* gene cluster converting urate to SCFA via 2,8-dioxopurine pathway. Antibiotic depletion elevates serum urate in healthy volunteers. PDB strains are naturallyLachnospiraceae-family taxa, already present in gut at variable abundance. This is a native, food-grade, mechanistically-distinct urate-lowering intervention separate from uricase. The OE wiki has no coverage of this. A targeted probiotic (specific PDB strain) or dietary prebiotic (fiber selectively feeding PDB taxa) is a Phase 0 intervention that doesn't require protein engineering.
