...
**comp-010 cassette compatibility update (2026-05-05):** In silico sequence analysis of the uricase (Q00511) + lactoferrin (P02788) payload pair found **no blocking cassette-design issues** for the proposed asymmetric architecture (direct-secretion uricase + glucoamylase-KEX2 lactoferrin). Overall cassette-design risk: **LOW** (Mechanistic Extrapolation). Two actionable design notes before gene synthesis: (1) Lactoferrin has a moderate-risk internal KEX2 site at mature position 579 (P1'=K) — monitor by SDS-PAGE in §1.9 for a ~67 kDa truncated band; if seen, mutate K597→Q in the codon-optimized gene. (2) *A. flavus* uricase (*uaZ*) ends in ...SKL (canonical PTS1 peroxisomal signal) — verify secretion by anti-uricase ELISA on culture supernatant vs. cell lysate; if misrouted, append C-terminal 3×Ala to mask the PTS1 signal. Combined ER disulfide burden = 1.06× Huynh 2020 adalimumab baseline (17 disulfides, all on Lf; uricase contributes zero), within demonstrated NSlD-ΔP10 capacity. Full analysis: [cassette-compatibility-computational.md](./cassette-compatibility-computational.md). (source: cassette-compatibility-computational.md)

**comp-011 C. utilis uricase cassette compatibility (2026-05-05):** The same seven-analysis pipeline was run on *C. utilis* uricase (P78609) + lactoferrin (P02788) in the Ward 1995 / *A. oryzae* architecture. Verdict: **MODERATE** (vs. *A. flavus* LOW per comp-010). Three manageable differences: (1) full codon optimization mandatory for *C. utilis* (AT-biased yeast GC~42% vs. *A. oryzae* GC~54%; CAI proxy 0.65 vs. 1.51 for *A. flavus*); (2) 4 free cysteines in *C. utilis* vs. 0 in *A. flavus* — risk of aberrant ER disulfide aggregation, mitigated by non-reducing SDS-PAGE QC; (3) 2 internal KR sites (positions 130 and 138, both HIGH) vs. 1 in *A. flavus* — non-load-bearing for direct-secretion design. The ALLN-346 ProteinGPS mutations (US10815461B2: I180V, V190G, Y165F, E51K, Q244K, I132R, A87G) should be incorporated into the codon-optimized synthesis. Platform recommendation: order both *A. flavus* and *C. utilis* variants as parallel direct-secretion cassettes for §1.9 — resolves the variant selection empirically at $0 additional fermentation cost. Full analysis: [c-utilis-uricase-cassette-compatibility-computational.md](./c-utilis-uricase-cassette-compatibility-computational.md). (Mechanistic Extrapolation; source: c-utilis-uricase-cassette-compatibility-computational.md)
