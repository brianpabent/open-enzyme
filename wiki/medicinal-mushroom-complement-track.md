...
- **Engineered koji cordycepin (cns1+cns2) + GLPP supplement** — the *engineered-koji-track* equivalent of the synergy pair. Koji's heterologous cordycepin lacks the natural pentostatin co-production unless the whole BGC is engineered in. May need pentostatin co-engineering OR GLPP supplementation as the ADA-inhibitor source. Open engineering question. **comp-023 (2026-05-14) metabolic-burden FBA: GREEN at Jeennor 2023 empirical titer (564 mg/L/d); breakpoint ~1,000× the empirical titer.** (Mechanistic Extrapolation; source: cordycepin-cassette-burden-computational.md)
- **Engineered koji cordycepin (cns1+cns2) + whole-fermentate *C. militaris* (native pentostatin)** *— added 2026-05-15.* The minimal-complexity cross-chassis pairing: engineered koji handles the bulk cordycepin production (metabolically tractable per comp-023's GREEN verdict at scale), and a small co-formulated dose of whole-fermentate *C. militaris* extract provides the ADA-protecting pentostatin (native co-evolved chemistry per Xia 2017, PMID 29056419). Avoids the additional cassette complexity of engineering pentostatin into koji, while inheriting *Cordyceps*'s natively-optimized cordycepin-to-pentostatin ratio. Distinct from the whole-fermentate-*Cordyceps*-alone bullet above because engineered koji can produce cordycepin at higher per-batch titers than cultivated *C. militaris* alone (comp-023's 564 mg/L/d vs. cultivated *Cordyceps* yields typically lower per-gram); the whole-fermentate *C. militaris* contribution here is dose-modest, sourced for its pentostatin (ADA inhibitor) not its bulk cordycepin. **Cross-chassis kinetics gated by [comp-025](./computational-experiments.md)** (ADA × cns1 substrate competition modeling, already queued in Planned Analyses — this combined-product strategy is one of its load-bearing decision branches). **Optional wet-lab gate:** a low-cost co-formulation stability test queued at [`validation-experiments.md` §2.7](./validation-experiments.md) (verify cordycepin stability in engineered-koji fermentate co-formulated with dried *C. militaris* extract, ADA-challenge assay readout). (Mechanistic Extrapolation; source: cordycepin-cassette-burden-computational.md + medicinal-mushroom-compound-mapping-computational.md)
