...
> **↳ Partial resolution (2026-07-14).** The load-bearing candidate — De Baere et al. 2013 HPLC-UV — is now **verified against the primary source directly** (PMID 23542733, [DOI](https://doi.org/10.1016/j.jpba.2013.02.032)): matrix-matched calibration on bacterial culture supernatant, linear 0.5–50 mM (r 0.9951–0.9993), LOQ 0.5–1.0 mM, underivatized (direct UV 210 nm). Downstream pages ([`open-questions.md`](./open-questions.md), [`computational-experiments.md`](./computational-experiments.md)) now re-anchor to De Baere directly rather than to a comp-038 §"Full-text verification" absent from the artifact. **Still open:** this comp-038 *artifact* remains the abstract-level first pass; the electrochemical (PMID 42041444) + SCFA-ELISA rejections are not in it and were not independently re-verified in this pass (treat as provisional); the empirical spike/recovery is the wet-lab gate at [`validation-experiments.md` §1.31](./validation-experiments.md).
