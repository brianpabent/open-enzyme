...
This page defines a predictive design framework for candidate koji cassettes and proposes a falsifiable test: a pairwise expression matrix with measured synergy coefficients. Demonstrated multi-cassette precedents are covered in [koji-endgame-strain.md](./koji-endgame-strain.md) §3 and [engineered-koji-protocol.md](./engineered-koji-protocol.md); the uricase + lactoferrin analysis in [cassette-compatibility-computational.md](./cassette-compatibility-computational.md) is one case study.
...
1. **Huynh 2020** (PMC7257131; Fungal Biol Biotechnol 7:7, [DOI 10.1186/s40694-020-00098-w](https://doi.org/10.1186/s40694-020-00098-w)) achieved 39.7 mg/L adalimumab in *A. oryzae* NSlD-ΔP10 only with the ten-protease-deletion background; the lower-protease-deletion strains (NSlDv10, AUT1-lD-v10-sD) and the control NSlD1 produced less, and wild-type RIB40 was used only as a DNA donor. The two cassettes encode heavy and light chains carrying **16 disulfides per assembled IgG1** (4 inter-chain + 12 intra-chain; UniProt P01857 and P01834 with standard IgG1 stoichiometry). The apparent ~40 mg/L ceiling for this PDI-heavy pair is consistent with, but does not prove, PDI/ERO1 saturation. (In Vitro)
2. **Wakai 2019** (Bioresour Technol 276:146-153, PMID 30623869, [DOI 10.1016/j.biortech.2018.12.117](https://doi.org/10.1016/j.biortech.2018.12.117)) achieved a **40× total cellulolytic activity uplift versus the single-integration strain** by stacking three cellulase cassettes at 5–16 chromosomal copies each and optimizing promoter/terminator combinations. Multi-copy integration produced approximately 10-fold uplift; promoter/terminator optimization supplied the remaining increase to 40-fold. The result must not be attributed to multi-cassette synergy. The three fungal-origin cellulases have modest disulfide and N-glycan loads, so the result is consistent with chaperone compatibility but does not isolate cross-cassette synergy from transcriptional scaling. (In Vitro)
...
## 3.5 Per-Architecture PDI Residence Time

The §3 bulk disulfide count treats all PDI/ERO1 substrates as fungible. That approximation is inadequate because PDI residence time per disulfide may vary by at least 3–4× across fold architectures:
...
- LF contains **16 disulfides** (32 mature-chain cysteines; Notari 2023, PMC10465537; see §10.2).
...
|---|---|---|---|
| **CCP / SCR / sushi** | DAF SCR1-4, Factor H CCPs | **0.3 – 0.6** | Geometrically pre-organized 2-disulfide scaffold; brief PDI engagement; fast cooperative folding per compact module. Schmidt 2010 (PMC2806952) provides structural/NMR evidence for rigid independent CCP units. Comp-030's ESM2 pseudo-pLDDT across 720 DAF SCR1-4 protein-distinct candidates is uniformly high (mean 88.8, std 0.5, range [87.6, 89.8], 100% above 80), consistent with sequence-robust architecture. [NUMBER PARTIALLY UNVERIFIED — no published per-domain PDI kcat; bounded from structural inference.] |
...
| **Serpin (metastable RCL-driven)** | C1-INH (SERPING1), α1-antitrypsin (SERPINA1), antithrombin III (SERPINC1) | **0.5 – 1.5** (PROVISIONAL — uncalibrated) | Low disulfide counts keep raw PDI load light, but the metastable fold may engage BiP/calnexin/PDI differently from calibrated classes. **The bound is provisional and requires koji-specific wet-lab calibration before any secreted-serpin design decision.** [NUMBER UNVERIFIED — structural prior only.] |
...
**Lactoferrin disulfide-count boundary:** Notari 2023 (PMC10465537) reports **16 disulfides** and 32 cysteines in mature human lactoferrin. P02788 contains 33 precursor cysteines because one lies in the cleaved signal peptide. Wally & Buchanan 2007 independently confirms that lactoferrin has fewer disulfides than serum transferrin. The architecture-adjusted model therefore uses 16; §10.2 documents the evidence chain.

### 3.5.4 Calibration set: §1.9 lactoferrin + §1.25 DAF SCR1-4
...
**Cross-references:** [`validation-experiments.md §1.9`](./validation-experiments.md), [`validation-experiments.md §1.25`](./validation-experiments.md). This calibration set is the predictive-power validation gate for the framework.
...
> **Cross-chassis combinations are outside this framework.** Compounds produced by a separate organism impose no chaperone load on the koji strain because they are not co-expressed there. The cordycepin row above scores the alternative *koji-engineering route*; its cytosolic enzymes remain outside the ER secretion/folding axis.

> **Scope note — what this framework tracks and what it doesn't.** The framework scores **ER folding-machinery competition** (PDI for disulfides, calnexin for N-glycans, BiP for general transit). It does **not** track generic proteome burden (ribosomes, amino acid pool, ATP for tRNA charging) imposed by cytosolic enzymes. For the current cassette stack, ER folding lanes are the binding constraint — koji's wild-type translation budget already supports ~30% secretion-dedicated output, so adding a ~400 aa cytosolic enzyme like cns1 is small change against the existing translation pool. If a future track stacks multiple cytosolic biosynthesis clusters, use a proteome-constrained metabolic model rather than extending this framework past its scope.
>
> **comp-023 boundary:** [`cordycepin-cassette-burden-computational.md`](./cordycepin-cassette-burden-computational.md) closes the plain-FBA cytosolic-burden question for the cns1+cns2 cordycepin row. The verdict is GREEN at the Jeennor 2023 empirical titer (564 mg/L/d), with the modeled burden breakpoint ~1,000× higher (~564 g/L/d). This framework covers ER folding; comp-023 covers cytosolic carbon, nitrogen, ATP, and NADPH burden. Both verdicts must hold for the full multi-payload configuration. Proteome-saturation modeling remains unresolved. (source: cordycepin-cassette-burden-computational.md)

The pattern is striking: the **koji multi-payload strain is implicitly already a chaperone-orthogonal design.** The five "molecules" of [koji-endgame-strain.md](./koji-endgame-strain.md) §1 distribute across non-overlapping load classes —
...
The current cassette set is chaperone-orthogonal across four load classes. Prospective candidates can therefore be scored against those classes, with underused machinery representing lower predicted competition.
...
| Multi-payload strain (uricase + Lf + native digestives + carnosine) | **~0.85 weighted** | **~0.75–0.90 weighted** | ↓ slight (range widens) | Lf's architecture-adjusted effective load (24–40) is higher than its bulk count (16) implied. Native digestives remain light. Uricase and carnosine remain at 0. The multi-payload strain's chaperone burden is dominated by Lf's transferrin-lobe architecture — which the bulk model underestimated. **Key insight:** the multi-payload strain without DAF SCR1-4 (i.e., the dual-cassette: uricase + Lf) is the tractable design — the whole Lf effective load question must be resolved by §1.9 first. |

The koji multi-payload strain's predicted weighted synergy ~0.85 — close to additive — is the framework's quantitative reframe of why the "five molecules in one strain" thesis is biochemically reasonable. It is *not* five-fold burden; it is roughly Huynh-2020-equivalent burden distributed across more deliverables.

---

## 5.5 Triple-Cassette Prospective Prediction — Uricase + Lactoferrin + DAF SCR1-4

This is a prospective test of whether the chaperone-orthogonal framework predicts beyond retrospective examples. The named koji hypothesis asks whether uricase + lactoferrin + DAF SCR1-4 can coexist in one strain or whether chaperone competition forces DAF onto a separate strain or the [engineered LBP chassis](./engineered-lbp-chassis.md).
...
**Triple-cassette bulk disulfide total: 24** (0 + 16 + 8). This is 1.50× the Huynh 2020 adalimumab reference of 16 disulfides. The dual pair is 16 = 1.00× by count. Adding DAF SCR1-4 raises bulk disulfide demand 50% above the dual/reference count. These ratios do not capture fold-specific PDI residence; the architecture-adjusted model below remains the decision model.
...
| **<0.6** | DAF cassette goes on a separate strain or the LBP chassis as a peer track. The CP0 closure thesis (H05) stays alive — only the chassis-route changes. | Maintain uricase + Lf dual-cassette as the multi-payload strain (CP1-CP6 coverage); route DAF SCR1-4 separately. **Assessed probability: high** — architecture refinement increases this probability relative to the bulk model. The central expectation now lands at 0.45–0.55, squarely in this gate. |
...
- **Design implication:** The predicted bottleneck is not total disulfide count but lactoferrin's prolonged PDI demand. Resolving it requires either host engineering that increases relevant PDI/ERO1 capacity or evidence that the Huynh antibody ceiling does not transfer to single-chain transferrin-lobe substrates.
...
### 5.5.8 Koji-Track Decision Tree

| Scenario | Probability | Koji-track direction |
|---|---|---|
| Synergy >0.85 (triple stacks cleanly) | Low | Pursue triple-cassette multi-payload strain as CP0+CP1-CP6 single-strain solution. Four cassette slots free in NSAR1 5-marker platform for downstream additions (kojA/kojR overexpression, carnosine module). |
...
| Synergy <0.6 (separate-strain routing) | High | Maintain uricase + Lf dual-cassette as the multi-payload strain. Route DAF SCR1-4 to either: (a) a dedicated second *A. oryzae* strain co-fermented with the dual-cassette strain, OR (b) the engineered LBP chassis ([engineered-lbp-chassis.md](./engineered-lbp-chassis.md)) as a parallel peer track. H05 thesis stays alive — only the chassis-route changes. The multi-payload strain stays at CP1-CP6 coverage (five chokepoints, four molecules per [koji-endgame-strain.md](./koji-endgame-strain.md) §1); CP0 direct-antagonism coverage becomes a separate-strain or peer-track output. |

**Key point in all three scenarios:** low triple-cassette synergy does not kill H05; it changes where the DAF SCR1-4 cassette is tested. The one-strain koji configuration is conditional, while the underlying CP0 payload hypothesis can move to another chassis.
...
2. **Uricase + Lf dual-cassette arm** — tests whether the pair, equal to the Huynh reference by bulk disulfide count but different in fold architecture, co-expresses without a capacity penalty. Required for interpreting the triple.
3. **Uricase + Lf + DAF SCR1-4 triple-cassette arm** — the prospective falsifiable test of this §5.5 prediction. **Optional for the §1.9 gate decision** (the dual-cassette arm is sufficient to decide whether to proceed with the multi-payload strain); adds ~$1-2k and 2-3 weeks but provides the framework's first prospective validation data point.

## 5.6 Cytosolic third cassette as a secretory-load bypass

§5.5 analyzes a triple-cassette strain where all three payloads are **secreted**. A separate design question is whether a cytosolic third cassette can bypass that secretory bottleneck.
...
Two overclaims must be avoided:
...
- The chaperone framework with explicit confidence bounds
...
For the named koji configuration, the relevant stacking variable is the count of *secreted* cassettes, not total cassette count. A cytosolic cassette may avoid ER competition, but its metabolic burden still requires separate validation.

This generalization is itself a candidate for codification once comp-028 returns and the pattern is empirically anchored.

Operationally applied at [`koji-endgame-strain.md` §3 "Third-cassette slot design rule"](./koji-endgame-strain.md) where the cytosolic-vs-secreted distinction gates which payloads route to the multi-payload strain vs. a separate-strain / LBP-peer-chassis. The framework's rule is independent of which specific payloads are currently active — payload selection is a strategy decision, the cytosolic-vs-secreted framing is a structural design rule.
...
**Generalization caveat.** The §3.5.4 calibration set validates only the lactoferrin transferrin-lobe and DAF SCR1-4 CCP/SCR fold classes in one host and format. Other secreted architectures remain bounded extrapolations until calibrated directly. Cytosolic payloads avoid this ER-folding bottleneck.

7. **Capacity-vs-titer benchmark ambiguity (flagged 2026-05-06; count corrected 2026-07-13).** The framework calibrates against Huynh 2020 adalimumab in NSlD-ΔP10 (39.7 mg/L, 16 disulfides), while Ward 1995 reported >2 g/L lactoferrin in *A. awamori* (also 16 disulfides per Notari 2023). Equal bulk count but radically different titer makes fold architecture, host, and culture format inseparable candidate explanations. The §1.9A Lf-only arm in NSlD-ΔP10 solid-state koji directly resolves which reference transfers. Until then, the coefficient ranges are hypotheses, not a demonstrated capacity law.

8. **No published DsbA/DsbC oxidative-folding capacity metric exists for the *E. coli* periplasm at 8–16-disulfide scale.** Comp-043 classified C1-INH as viable, DAF SCR1-4 as provisional, and lactoferrin as not viable for EcN, but the capacity band is inferred rather than measured and partly anchored to a compartment-mismatched cytosolic system. EcN folding claims for 8+-disulfide payloads therefore remain provisional pending a direct capacity or isomerization-rate assay.
...
- [koji-endgame-strain.md](./koji-endgame-strain.md) — The 4-molecule / 5-chokepoint multi-payload design evaluated by this framework
...
- [manual-literature-mining.md](./etc/manual-literature-mining.md) §"Pre-commit verification gate" — verification method used for §10.1

### 10.1 Evidence verification

The [pre-commit verification gate](./etc/manual-literature-mining.md) was applied to the three load-bearing numbers feeding §1 and §7:
...
Huynh 2020 was checked against full text. Wakai 2019 and Zhang 2006 were checked at abstract level; methodological details not present in those abstracts require full-text verification before downstream reliance.

### 10.2 Architecture-coefficient evidence
...
**LF disulfide-count boundary (§3.5.3):** Notari 2023 (PMC10465537) explicitly reports **16 disulfides** and 32 cysteines after complete reduction of mature lactoferrin. The extra cysteine in the UniProt precursor lies in the cleaved signal peptide; architecture-adjusted computations use 16.

**Multilingual evidence boundary:** J-STAGE, CNKI, and J-GLOBAL searches identified no additional non-English primary studies of lactoferrin or SCR/CCP oxidative folding kinetics beyond PubMed-indexed sources. The architecture coefficients therefore rely on English-language primary sources and do not invoke the translation cross-check protocol.

---

## 11. Implications for multi-cassette koji design

The chaperone-orthogonal framework addresses a practical design question: how many cassettes can one chassis support before shared folding machinery becomes limiting?

The reframe: **not "how many" but "which combinations get synergistic yield."** Concretely, the OE roadmap implications:

1. **The multi-payload strain's "five molecules" thesis is biochemically reasonable**, not greedy. Total chaperone burden ≈ Huynh 2020 antibody baseline because four of the five payloads are off-pathway or chaperone-light. The framework converts what looked like an aggressive payload count into a defensible engineering target.
...
3. **NSlD-ΔP10 is the leading host intervention; cross-class chaperone-helper combinations are the leading capacity-expansion experiment.** Zhou 2016 found that constitutive HacA<sup>i</sup> suppresses native amylase via RESS feedback, making it a poor default where native amylase output matters. The alternative is to measure baseline titers and per-class bottlenecks before installing matched helper-pair cassettes.

4. **The pairwise expression matrix is the central characterization experiment** for this framework. Its value is a reusable, measured basis for later cassette decisions rather than continued guess-and-check expression.

5. **The literature gap is substantive.** No published pairwise expression matrix was identified for koji, and no published predictive rule was identified for chaperone-class orthogonality in a filamentous fungus. The component evidence exists, but the composed framework remains an unvalidated research hypothesis.

6. **The "five-molecule multi-payload strain" is more biochemically defensible than it looks.** The Wakai 2019 3-cellulase result (40× total cellulolytic activity over single-integration baseline; ~10× from copy-stacking + ~4× from promoter/terminator engineering, verified 2026-05-06) is direct *A. oryzae* evidence that three light substrates *can coexist at high titer without titer collapse* in the same chaperone subset — a necessary precondition for chaperone-orthogonal compatibility, though not direct evidence of cross-cassette synergy distinct from per-cassette transcriptional scaling. The koji multi-payload strain has *more* orthogonality than Wakai (substrates partitioned across BiP/PDI/calnexin/cytosolic/off-pathway classes vs. Wakai's three substrates all in one class). Predicted synergy ≥0.85 is consistent with the absence-of-titer-collapse evidence in Wakai and the broader chaperone-orthogonality framework.
