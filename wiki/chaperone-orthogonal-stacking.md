...
**Note on lactoferrin disulfide count correction:** The OE wiki previously used "17 disulfides" for lactoferrin in several pages. Notari 2023 (PMC10465537) explicitly states **16 disulfides** and 32 cysteines after complete reduction of mature human lactoferrin. The canonical P02788 precursor sequence contains 33 cysteines total because one lies in the cleaved signal peptide; counting the full precursor created the earlier discrepancy. Wally & Buchanan 2007 independently confirms LF has fewer disulfides than serum transferrin. This architecture-adjusted model therefore uses 16 throughout; §10.2 documents the verification chain.
...
**Cross-references:** [`validation-experiments.md §1.9`](./validation-experiments.md), [`validation-experiments.md §1.25`](./validation-experiments.md), [`synthesis/strategic-reflections/`](../synthesis/strategic-reflections/) (chaperone-framework predictive-power validation entry — this calibration set is the operational instantiation of that reflection's trigger condition).
...
8. **No published DsbA/DsbC oxidative-folding capacity metric exists for the *E. coli* periplasm at 8–16 disulfide scale — the EcN-chassis analogue of the koji-side gaps above (added 2026-07-13, comp-043).** The engineered-LBP peer track ([`engineered-lbp-chassis.md`](./engineered-lbp-chassis.md)) uses *E. coli* Nissle's periplasmic DsbA/DsbC system as its disulfide-folding machinery — the bacterial counterpart to koji's ER PDI/ERO1. [comp-043](./daf-lactoferrin-ecn-folding-feasibility-computational.md) asked how far that scales and found the plausible→not-plausible crossover at **DAF SCR1-4 (8 disulfides)**: C1-INH (2) VIABLE, DAF SCR1-4 (8) PROVISIONAL, lactoferrin (16) NOT-VIABLE. The verdict is *provisional* precisely because the DsbA/DsbC capacity band (conservative 5 / moderate 8 / optimistic 12 effective-demand units) is inferred from *E. coli* periplasmic-expression precedent (Fab-class secretion; SHuffle strains), not measured — and the optimistic anchor is compartment-mismatched (SHuffle folds disulfides in the cytoplasm, not the secretory periplasm an LBP luminal payload routes through), biasing the honest read conservative. **The highest-leverage missing measurement is a DsbA/DsbC oxidative-folding capacity / isomerization-rate assay at 8–16 disulfide scale** — the EcN-side counterpart to the §3.5.4 koji α-coefficient calibration gap (item 6) and the §1.9 capacity-vs-titer ambiguity (item 7). Until it exists, any EcN folding claim for an 8+-disulfide payload stays provisional. This is why lactoferrin belongs on koji (>2 g/L precedent, Ward 1995) and DAF SCR1-4 stays primarily on koji with EcN only a provisional secondary route.
...
**LF disulfide count correction note (§3.5.3):** The OE wiki used "17 disulfides" for lactoferrin in prior pages. The primary source, Notari 2023 (PMC10465537), explicitly reports **16 disulfides** and 32 cysteines after complete reduction. UniProt annotation created the earlier counting discrepancy. The 16-disulfide value is used throughout the current corpus and architecture-adjusted computations; the propagation was completed 2026-07-13.
