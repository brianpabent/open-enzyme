...
4. **ChEMBL cross-check should become a standing rigor tool, not a one-off compound appendix.** *Supported.*
   ...
   **✓ Actioned 2026-04-27:** Already wired up — `.github/workflows/chembl-refresh.yml` runs cron `0 12 1 1,4,7,10 *` (12:00 UTC on the 1st of Jan/Apr/Jul/Oct, aligned with ChEMBL's release cadence), pulls `scripts/chembl-refresh-prompt.md`, runs on `claude-sonnet-4-6`. Has a `workflow_dispatch` trigger with a free-text note input for off-cycle runs (e.g., "ChEMBL v35 released early"). Concurrency group is distinct from `wiki-sweep` so they don't block each other; `[skip ci]` on refresh commits prevents self-trigger. Closed via commit `0831c24` ("Closes #23"). No further setup needed; next scheduled run is 2026-07-01.
...
1. **Beta-caryophyllene + BHB is the evidence-strongest non-pharma gout NLRP3 combo — arguably more than oridonin + BHB.** *Supported.*
   ...
   **✓ Actioned 2026-04-27:** Already triaged via sequencing across the wiki — the order-of-magnitude rodent gap is what gates the spend, not a blocker. (1) [`cannabinoids-terpenes.md` §8 Experiment #1](./cannabinoids-terpenes.md) runs BCP human THP-1 IC50 first ($1k–1.5k) — BCP has zero curated direct human NLRP3 bioactivities in ChEMBL v34, so this generates the species-bridging measurement and decides whether the rodent data translates. Also in [`validation-experiments.md` §1.7](./validation-experiments.md#17-nlrp3-inflammasome-pathway-validation-thp-1-msu-macrophage-assay) compound panel. (2) [`cannabinoids-terpenes.md` §8 Experiment #5](./cannabinoids-terpenes.md) is the BCP+oridonin combination in MSU model ($3k–5k), conditional on Step 1 — framed as dose-reduction of oridonin's Cys279 covalent risk, not head-to-head with BHB. The original "BCP+BHB vs. oridonin+BHB" framing of this synthesis Connection collapses into Experiment #5 because BHB orthogonality is already settled at the stack level in [`oridonin.md` line 150](./oridonin.md), [`bhb-ketones.md`](./bhb-ketones.md), and [`supplements-stack.md`](./supplements-stack.md): BHB (K⁺ efflux) + BCP (CB2/TLR4) + oridonin (Cys279) hit distinct molecular touch-points, "orthogonal additivity expected." (3) [`validation-experiments.md` §1.19 standing methodology](./validation-experiments.md#119-methodological-standard--rodent-cellular-ic50-translation-caveat) names BCP as a mouse-only compound requiring human-cell follow-up before evidence-tier promotion — the discipline Brian's annotation was asking for.
