...
**7. Metabolic-overload mechanism testability.** The TCM-derived candidate mechanism #5 — exertion → organ-function fatigue → impaired urate excretion → transient local supersaturation — has not been tested in modern kinetic terms. Does sustained exertion measurably impair renal urate clearance in gout patients during the post-activity window in which flares occur? Serial spot urinary urate / creatinine ratios across an exertion-then-rest period in DECT-confirmed gout patients would directly address this. The mechanism would be testable without requiring TCM ontology buy-in — just as a renal-clearance kinetic question. **n=1 self-experiment form landed 2026-05-21:** [`validation-experiments-archive.md` §3.11](./etc/validation-experiments-archive.md) — single-phase exertion challenge (30 min weighted walking) with paired serum UA + urinary urate/Cr sampling, pre-registered predictions discriminating mechanism #5 from mechanisms #1–4. Doesn't substitute for the DECT-confirmed cohort study above, but is the cheapest first-pass kinetic test.

**Compound uncertainty with the Q141K butyrate worked example (added 2026-05-22 per sweep 2026-05-20 Contradiction #1).** When a Q141K-positive subject following [`genotype-informed-supplement-workflow.md`](./genotype-informed-supplement-workflow.md)'s butyrate-emphasis stack flares after an exertion event, the metabolic-overload mechanism (#5) interacts with comp-038's documented Tier 2 butyrate input-verification gap to produce a four-way n=1 attribution problem: (a) butyrate intervention failing, (b) butyrate dose wrong but unverifiable, (c) exertion-trigger mechanism is mechanical (one of #1–4) rather than metabolic-overload (#5), (d) regression to the mean. The exertion-challenge protocol with serial spot urinary urate/Cr ratios (now in `etc/validation-experiments-archive.md` §3.11) is what disambiguates (c) from (a/b/d) by directly measuring renal urate kinetics — without it, a post-exertion flare in a Q141K butyrate-stack subject cannot inform which mechanism dominated. The workflow page now documents this compound uncertainty in its Q141K worked example so subjects executing the protocol can plan exertion-exposure logging + post-exertion biomarker draws as part of the self-experiment design rather than discovering the attribution gap retrospectively.
