...
**7. Metabolic-overload mechanism testability.** The TCM-derived candidate mechanism #5—exertion → organ-function fatigue → impaired urate excretion → transient local supersaturation—has not been tested in modern kinetic terms. Serial urinary and serum urate measurements around a controlled exertion window could distinguish renal-clearance kinetics from purely mechanical trigger hypotheses. A safe, interpretable protocol would require separate design and review before execution.

**Compound uncertainty with the Q141K butyrate example.** A post-exertion flare cannot by itself distinguish intervention failure, dose uncertainty, a mechanical trigger, a metabolic trigger, or regression to the mean. Direct renal-urate kinetics would be needed to make that attribution.
