...
> **Calibration note (reset 2026-07-13):** No validated model currently predicts the serum-urate effect, genotype ordering, or adequate dose of the engineered gut-lumen UOX approach. [comp-044](./gut-lumen-uricase-physiologic-regime-computational.md) invalidated comp-019's numerical claims after restoring physiological substrate occupancy and residence time. Q141K remains a sensible prospective stratification variable, but it does not currently justify a response prediction or a single 25 mg/day dose. Treat all engineered-strain entries as future research. Genotyping still matters for established clinical reasons such as HLA-B\*58:01-associated allopurinol hypersensitivity and transporter-related baseline urate handling; see [personal-genome-protocol](./personal-genome-protocol.md).
...
### Combined-route flare protocols — mechanism-non-redundant stacking (added 2026-05-19; Protocol C added 2026-05-21)

Three named combined protocols for active flare. Protocols A and B leverage **mechanism-non-redundant convergence on NLRP3** through independent receptor pathways and delivery compartments — both are all-suppression compositions. Protocol C adds a **resolution-class arm** (specialized pro-resolving mediators via DHA-emphasis omega-3), crossing two distinct mechanism *classes* (suppression + active resolution) rather than just stacking additional suppression routes. *[All three are Speculative; mechanism-composition logic, no human gout RCT for any combination as named.]*
...
**Protocol C — Triple-mechanism acute-flare stack (colchicine + topical CBD:THC + DHA SPM resolution arm).** The first composition that crosses two mechanism classes — two NLRP3-suppression arms plus an active-resolution arm. A gout flare unfolds through three overlapping time phases — onset (minutes-hours), amplification (hours-days), resolution (days) — each of which engages multiple chokepoints (CPs) from the [`nlrp3-exploit-map.md`](./nlrp3-exploit-map.md) framework. The phase-to-CP mapping is not 1:1: CP2 spans onset and amplification; CP6 spans amplification and resolution. Protocols A and B aggressively suppress the onset/amplification phases; Protocol C keeps those suppression arms and adds an active resolution arm that accelerates the wind-down phase. Composition + timing:
- **t = 0:** Oral colchicine 1.2 mg + topical 1:1 CBD:THC on the affected joint + ice cycling (suppression arms, same dosing as Protocol A)
- **t = +1 h:** Oral colchicine 0.6 mg (per AGREE trial)
- **Throughout:** Continue DHA-emphasis omega-3 at the existing chronic dose (do not load during the flare itself — DHA → SPM biosynthesis is gated on baseline omega-3 index, not acute intake)
- Mechanism arms by class:
  - **Suppression — colchicine** → β-tubulin → CP3 ASC blockade + CP2 P2X7 inhibition (intracellular, systemic; Leung 2015 PMID 26228647 for the CP2 dual-hit)
  - **Suppression — topical CBD:THC** → CB2 GPCR on synovial macrophages → CP2 NLRP3 conformational suppression (plasma-membrane, local)
  - **Resolution — DHA-emphasis omega-3** → 17-HDHA → RvD1/MaR1 SPMs → CP5b ALX/FPR2 + LGR6 active-resolution signaling → M1→M2 macrophage switch, neutrophil-stop-signal, aggNET-mediated C5a degradation (systemic, resolution class — see [`spm-resolution-pathway.md`](./spm-resolution-pathway.md) §3.2, §3.4, §5)

**CP coverage map — which Protocol C arm hits which chokepoint in which phase:**

| Phase | Time scale | CPs engaged in this phase | Protocol C coverage | Gaps in Protocol C (composes with) |
|---|---|---|---|---|
| **Onset** | minutes–hours | CP0 (complement priming on crystals → C5a), CP1a/b (NF-κB transcriptional + C5a→ROS non-transcriptional priming), early CP2 (K⁺ efflux begins) | Topical CBD:THC → CP2 (partial) | CP0 + CP1 uncovered — composes with upstream-CP0 dietary candidates (rosmarinic acid, luteolin, *Houttuynia* HCP/HCPM, lactoferrin per [`complement-c5a-gout.md`](./complement-c5a-gout.md) §9) and CP1 priming blockers (Boswellia / quercetin / BHB) |
| **Amplification** | hours–days | Full CP2 (NLRP3 assembly), CP3 (ASC speck), CP4 (caspase-1), CP5a (IL-1R feedforward), CP6a (5-LOX/LTB4 neutrophil amplification), CP6b (GSDMD pyroptotic exit) | Colchicine → CP3 + CP2 (P2X7); CBD:THC → CP2 (sustained) | CP4 / CP5a / CP6a / CP6b uncovered — composes with caspase-1 inhibitors, IL-1Ra (anakinra at CP5a per §"Anakinra SC for acute flare"), 5-LOX inhibitors (AKBA, quercetin at CP6a), GSDMD inhibitors (disulfiram, DMF at CP6b) |
| **Resolution** | days | CP5b (SPM → ALX/FPR2 + LGR6 active resolution), late CP6 (neutrophil apoptosis, efferocytosis, aggNET formation per Schauer 2014 PMID 24784231) | DHA-emphasis omega-3 → CP5b (direct); late CP6 indirect via SPM signaling | None major — CP5b is the load-bearing resolution chokepoint and Protocol C covers it; the falsifiable C5a-decline-slope prediction at [`spm-resolution-pathway.md` §7.3](./spm-resolution-pathway.md) is the mechanism signature for this arm |

- **Distinguishing feature:** Protocols A and B *block* the cascade; Protocol C blocks the cascade *and* commands resolution. The DHA arm is gated on chronic omega-3 loading (target blood omega-3 index ≥ 8%) and produces a falsifiable mechanism signature — accelerated serum C5a decline during flare resolution per [`spm-resolution-pathway.md` §7.3](./spm-resolution-pathway.md).
- **Evidence tier:** Colchicine arm Clinical Trial (AGREE); CBD:THC arm In Vitro / Animal Model; DHA SPM arm In Vitro / Animal Model with human falsifiable prediction; combination as named is Speculative.
- **N = 1 prospective protocol:** [`self-experiment-protocol.md` §13](./self-experiment-protocol.md) lays out mechanism-separated endpoints (per-arm timing windows + composite resolution endpoint) so the three arms can be evaluated independently rather than collapsed into one "flare better" outcome.

**Which protocol fits which user:** Protocol A is for users already on colchicine with no contraindications (renal function adequate, no statin/macrolide/cyclosporine drug-interaction surface). Protocol B is for users on prednisone (acute or taper) where the colchicine drug-interaction surface is unacceptable or where colchicine isn't part of the regimen. Protocol C is the same user profile as Protocol A *plus* the user already maintains a chronic DHA-emphasis omega-3 dose at a target omega-3 index ≥ 8% (no acute-loading benefit — the resolution arm is gated on chronic state). All three are layered onto whatever urate-lowering therapy is in place; all three target acute-flare inflammation, not chronic hyperuricemia.
...
The push-time knowledge update must propagate material compound changes from the evidence catalog to this application surface.
