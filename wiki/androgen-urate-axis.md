...
- **Testosterone upregulates URAT1 mRNA (NOT protein in non-orchiectomized animals); Smct1 protein induction + GLUT9 attenuation are the actual protein-level drivers of testosterone-driven male hyperuricemia in mouse renal tissue.** [VERIFIED-AGAINST-PRIMARY 2026-05-07 — Hosoyamada/Takiue 2010, PMID 20589576: in orchiectomy-replacement context, both URAT1 mRNA and protein restored by T. **Crucial nuance** (refined 2026-05-07 post comp-017 full-text re-read): in non-orchiectomized animals (closer to physiological-male baseline), testosterone enhances Urat1 *mRNA* but Urat1 *protein* level was unaffected. The same paper showed testosterone enhances Smct1 (sodium-coupled monocarboxylate transporter; drives urate reabsorption indirectly via lactate/nicotinate co-transport) at both mRNA and protein levels, **and attenuates GLUT9** — making Smct1 protein induction + GLUT9 attenuation the proximate protein-level renal mechanism drivers, not URAT1 protein. The "T → URAT1 ↑" headline framing this page (and others) inherited from secondary-literature is mRNA-only at physiological-male baseline. See [comp-017](./intestinal-abcg2-sex-dimorphism-public-data-mining-computational.md) for the full-text re-read that surfaced the GLUT9 attenuation finding.] (Animal Model; rodent renal expression studies.)
...
> **Counter-agent — fermentable fiber via PPARγ:** Butyrate-driven PPARγ induction of wild-type ABCG2 remains a relevant lever. The earlier statement that butyrate itself rescues Q141K through the Basseville mechanism was an attribution error: Basseville 2012 established rescue with pharmacological/chemical-chaperone perturbations, not direct butyrate rescue. Q141K trafficking and functional urate flux under butyrate now require validation §1.14.
...
- **Cross-track URAT1 redundancy — emergent designed-coverage across three Open Enzyme tracks (added 2026-05-08).** The carnosine URAT1 lever in the koji track has *mechanistically equivalent* counterparts in the medicinal-mushroom and TCM tracks, providing platform-level redundancy that none of the tracks were designed for individually:
  ...
  All three target the same chokepoint (renal URAT1 reabsorption) by the same mechanism class (expression-level downregulation), via independent compound classes from independent producer organisms. **Importantly, this is *emergent* designed-coverage** — none of the three tracks was scoped against the others; the convergence is platform architecture surfaced retroactively. **Resilience implication:** any patient who cannot use one track (koji intolerance, mushroom-allergy, TCM-availability gap) has a mechanistically equivalent URAT1 option in another track.

  **Caveat — koji's URAT1 coverage is *proposed*, not yet demonstrated.** Carnosine in the koji track is the *optional third cassette* per [koji-endgame-strain.md §2.5](./koji-endgame-strain.md), gated on §1.24's wet-lab feasibility test (target ≥500 mg/L titer). If §1.24 reports below threshold, koji's URAT1 coverage is not a current platform feature; it would require alternative carnosine-delivery routes (oral supplement, separate strain) or new cassette engineering. The mushroom-track (cordycepin) and TCM-track (astilbin) URAT1 coverage are independent of this contingency. **Engineering nuance:** carnosine's third-cassette feasibility is *titer-gated*, not chaperone-load-limited — carnosine biosynthesis is cytosolic (CarnS + panD, zero secretion-pathway burden, zero disulfides), so the chaperone-orthogonal stacking framework ([chaperone-orthogonal-stacking.md](./chaperone-orthogonal-stacking.md)) actually FAVORS adding it to the dual-cassette uricase + lactoferrin endgame strain (predicted weighted synergy ≥0.85 per [koji-endgame-strain.md §407](./koji-endgame-strain.md), Huynh 2020 adalimumab-equivalent ER burden). The disulfide-load concern that gates triple-cassette stacking applies to **DAF SCR1-4 SCR1-4** (8 additional disulfides on PDI) routed to separate-strain per the §1.25 wet-lab gate, NOT to carnosine. Items 5 and 11 of the 2026-05-08 walkthrough have the chaperone-framework calibration-set context.
...
- **Labadzhyan A et al. "Lipoprotein(a), SHBG, and insulin sensitivity."** Not searched in this verification pass (lower priority—supports SHBG framing, not a load-bearing claim for the androgen-urate track).
