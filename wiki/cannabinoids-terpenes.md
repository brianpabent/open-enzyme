...
- **GI concentration question answered:** The low systemic bioavailability does NOT automatically mean high luminal concentration. Unabsorbed CBD is not retained as active free drug in the gut lumen — but the mechanism is **lipid/fecal-matrix sequestration + gastric-acid lability + extensive host metabolism**, *not* primarily colonic microbial degradation as an earlier draft of this page stated (corrected 2026-07-13 per the CBD-vs-flavonoid gut-degradation scan — direct colonic-microbial degradation of CBD is only weakly evidenced; the gastric-acid conversion to THC is stomach-specific, ~85% at 60 min, with no conversion in neutral buffer, [Merrick 2016, PMID 28861485](https://doi.org/10.1089/can.2016.0020)). The "poor bioavailability = gut-lumen concentration" hypothesis is not supported by available PK data. If gut-lumen anti-inflammatory activity is the goal, this is not the right PK profile. **Note:** CBD is not an ABCG2/BCRP inhibitor (its targets are P2X7 / NLRP3), so unlike the flavonoids it was never a gut-lumen-sink concern for transporter reasons — see the per-compound stratification in [`abcg2-modulators.md` §"Gut-luminal metabolic stability"](./abcg2-modulators.md).
...
**CP0 intervention surface bracketing (added 2026-05-22, per sweep 2026-05-21 Connection #4 + Priority Action #2).** This four-route layered approach is the **suppression-class extreme** of the CP0 intervention surface — every arm acts downstream of complement activation (NLRP3 dampening via GR, CB2, TRPV1 desensitization; ice-mediated neutrophil chemotaxis reduction). At the opposite end of the same intervention surface sits the dormant **C1-INH (LBP-luminal) + rosmarinic acid** composition documented at [`complement-c5a-gout.md` §9.9](./complement-c5a-gout.md) — a **blockade-class** approach that would prevent complement activation at two upstream nodes (classical/lectin pathway entry via C1-INH, C3 convertase via rosmarinic acid) rather than suppressing its downstream consequences. The two compositions bracket the CP0 design space: suppression-dominant vs blockade-dominant. The platform's CP0 strategy is currently entirely suppression-class during active flares (Protocols A/B/C in [`gout-action-guide.md`](./gout-action-guide.md)) with zero active resolution-class CP0 coverage — making the bracketing visible sharpens the CP0 research agenda + clarifies what the dormant §9.9 composition would add if both of its arms cleared individual validation gates.

**Cross-references:** [`gout-action-guide.md` §"Combined-route flare protocols"](./gout-action-guide.md) Protocol B (the formalized version of this n=1 observation); [`nlrp3-exploit-map.md` §"Glucocorticoid Receptor (GR) Signaling"](./nlrp3-exploit-map.md) (the prednisone arm's mechanism documentation); [`complement-c5a-gout.md` §9.9](./complement-c5a-gout.md) (dormant C1-INH + rosmarinic acid composition — blockade-class CP0 bracket counterpart to this suppression-class observation); [`logs/cortisol-fasting-glucocorticoid-inflammasome-lit-scan-2026-05-19.md`](../logs/cortisol-fasting-glucocorticoid-inflammasome-lit-scan-2026-05-19.md) (the H2 lit scan that established the GR / NLRP3 / MSU mechanism in vivo).
