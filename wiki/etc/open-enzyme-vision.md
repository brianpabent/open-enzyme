...
>
> The "engineered enzymes in koji" framing in this section is the platform's first and primary chassis expression — but it is not the entire mission. The 2026-05-05 commit of [`engineered-lbp-chassis.md`](../engineered-lbp-chassis.md) opened a peer track (engineered Live Biotherapeutic Products — obligate-anaerobe colonic residents like *F. prausnitzii*) that serves the same gout-solving mission via a fundamentally different chassis class (commercial-pharmaceutical, FDA LBP regulatory path, not home-fermentable). The 2026-05-06 commit of [`medicinal-mushroom-complement-track.md`](../medicinal-mushroom-complement-track.md) opened a fifth peer track (native-compound medicinal mushrooms — *G. lucidum* GLPP, *C. militaris* cordycepin+pentostatin, *P. citrinopileatus* ergothioneine) that requires **zero genetic engineering** — cultivation + extraction only, with the lightest regulatory burden of any peer track (GRAS food / supplement-grade).
>
...
### Platform Choice — Koji-Primary Chassis, with Yeast Retained for Specific Modules
...
**Update 2026-05-16 from [comp-033](../computational-experiments.md) + [comp-036](../computational-experiments.md):** single-dose inhaled mRNA-IL-1RA returned **RED on anakinra-Cmax-equivalence**; repeat-dose follow-up returned **YELLOW on receptor-occupancy framing** at corrected IL-1Ra Kd 0.1–10 nM (Arend 1990, Schreuder 1997 — nM regime, not pM). BID × 4–28 doses is the best regimen — median 50–56% of flare window above 80% occupancy — but doesn't clear high-confidence GREEN bar (95% of window) at current input uncertainty. Two wet-lab measurements would tip the verdict: (1) translation-efficiency mass ratio in human alveolar epithelium; (2) modern SPR Kd measurement.

**Critical operator-relevant reframe (load-bearing):** the right clinical comparison is NOT "can inhaled mRNA match anakinra Cmax?" but **"is partial receptor-occupancy × cleaner side-effect profile worth it vs prednisone over recurring flares × decades?"** Prednisone's cumulative burden (bone, cataracts, adrenal, glucose, mood) accumulates over years of recurrent gout regardless of efficacy. Repeat-dose inhaled mRNA at 50% flare-window occupancy may not "abort" a flare like anakinra but may meaningfully shorten/dampen with negligible cumulative steroid-class burden. Partner conversations should lead with **prednisone-displacement** (side-effect profile + cost edge), not anakinra-equivalence. Sensitivity dominated by Kd_nM (ρ −0.69) + translation-efficiency mass ratio (ρ +0.58).

**Critical commercial-strategy clarification:** the partner conversation should NOT lead with gout. The IL-1 axis is implicated across COPD exacerbations, severe T2-low asthma, ARDS, IPF, CRS from CAR-T, recurrent pericarditis — all with much larger markets than gout. The regulatory strategy that actually works: approve for a primary respiratory/inflammation indication first (most likely COPD or ARDS by market size × mechanism fit), then off-label use spreads to gout — same playbook as anakinra (approved 2001 RA, now widely off-label for gout / pericarditis / sJIA / CAPS / COVID-CRS). OE's discovery-engine contribution is **target validation + the multi-indication cross-leverage argument** that makes a cassette-swap platform-justifiable to a Tier-A inhaled-mRNA company (Arcturus, ReCode, Ethris, Sanofi/Translate Bio). Gout patients access via off-label after primary-indication approval. Near-term bridge: anakinra SC × 3 days for acute flare per [gout-action-guide.md](../gout-action-guide.md).
