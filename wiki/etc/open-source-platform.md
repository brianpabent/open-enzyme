...
**Empirical validation.** The cross-vendor heterogeneity guard has produced two documented catches: the DAF SCR1-4 disulfide-count incident (2026-05-06, caught at the pre-commit grep-verify gate — see [`etc/manual-literature-mining.md` §"Pre-commit verification gate"](./manual-literature-mining.md#pre-commit-verification-gate-the-rule-that-catches-errors-before-the-sweep-not-after)) and the Paperclip Km misreport (2026-05-13, caught by DeepSeek V4-Pro's Pass 4 review nine days after entering the corpus and propagating into an external paper draft — see [`etc/paperclip-deep-dive.md` §"2026-05-13 correction"](./paperclip-deep-dive.md)). The two incidents are complementary: DAF is the canonical case for catching errors at the wiki-authoring step before they propagate; Paperclip is the canonical case for catching errors that survived single-model generation and review. Full write-up of both incidents: [`papers/cross-vendor-heterogeneity-guard/draft.md`](../../papers/cross-vendor-heterogeneity-guard/draft.md) (in progress).
