...
The library is one of two parallel outputs of the Open Enzyme project. The other is a **discovery engine** — a chokepoint-based methodology for mapping every vector that causes, treats, or mitigates a given disease, applied first to gout. The discovery engine produces a structured cascade map plus a repurposing surface (FDA-approved drugs that hit relevant chokepoints but were never clinically tested for the target disease). The strain library is the **highest-priority chassis** synthesized from the discovery engine — the engineered-koji platform converged from the gout vector mapping because koji harmoniously hits multiple chokepoints in one strain (uricase + lactoferrin + carnosine + DAF SCR1-4) and matches the food-grade / open-source / democratized-access positioning. **It is one chassis-shaped expression of the mission, not the mission itself.** (source: open-enzyme-vision.md, §1–2)

**Discovery-engine outputs that don't fit the koji chassis** — kidney-tropic siRNA, engineered LBPs, PDB via engineered EcN, mRNA-IL-1RA, intra-articular formulations, phage modulation — are tracked as [chassis-pending interventions](../chassis-pending-interventions.md) rather than deprioritized. Chassis is downstream of chokepoint; the food-grade open-source positioning is a strategic choice for *how OE solves what it solves*, but the chokepoints don't bend to the chassis — when an intervention needs a different chassis, OE's role shifts (discovery-engine output, partnership, spinout) rather than abandoning the intervention. See [`synthesis/strategic-reflections/2026-05-15-chassis-is-downstream-of-chokepoint.md`](../../synthesis/strategic-reflections/2026-05-15-chassis-is-downstream-of-chokepoint.md).
...
The three-pass sweep daemon (Pass 1 Propagate → Pass 2 Synthesize → Pass 3 Review, currently DeepSeek V4-Pro / DeepSeek-or-Gemini / OpenAI GPT-5.5) currently operates only on the wiki itself. [Paperclip](./paperclip-deep-dive.md) — an MCP server exposing ~11M full-text papers (PMC + arXiv + bioRxiv/medRxiv) + ~150M abstracts (OpenAlex) — was evaluated as a potential sweep-pass augmentation to produce a "literature delta" surface. **Decision: do not integrate.** A 2026-05-05 verification test (uricase variant landscape, ~12 papers with known-correct ground truths) revealed that the `map` operator — the synthesis primitive that would power any automated extraction — systematically hallucinates quantitative data and misattributes organisms. Concrete examples: wrong organism identity (A. flavus → A. globiformis), fabricated kinetic parameters (~7.5× off; corrected from earlier "7,500×" documentation error 2026-05-13 — see [paperclip-deep-dive.md](./paperclip-deep-dive.md) §"2026-05-13 correction"), and invented wet-lab data for a purely computational paper. Wiring this into the sweep would inject a structured external hallucination source into a corpus designed for PhD-grade rigor, exactly the failure mode the multi-model synthesis architecture is meant to guard against. Paperclip remains a manual-research-only tool, used interactively with verification discipline (grep-verify every load-bearing number; anchor identity claims to `meta.json`; never propagate `reduce` summaries). **Reopen condition:** if GXL ships a verified upgrade of the `map` reader model and the uricase variant probe passes cleanly, revisit. (source: paperclip-deep-dive.md; Mechanistic Extrapolation)

#### Multi-model synthesis as guard against epistemic homogenization

A knowledge graph maintained by a single synthesizer converges, over time, on that synthesizer's blind spots and biases — the corpus starts to encode one model's prior rather than the underlying biology. This concern was surfaced by the system itself: in the [2026-04-25 DeepSeek V4-Pro peer-review pass](../../logs/v4-peer-review-2026-04-25-deepseek.md) (Connection 7), a DeepSeek model reviewing the Claude-Opus output flagged that running cheap full-corpus sweeps on a single model would erode exactly the diversity that makes the corpus trustworthy. That a model from a different vendor caught a methodological risk Claude had not surfaced unprompted demonstrates why heterogeneity earns its keep. Model assignments have rotated through Anthropic, Google, OpenAI, DeepSeek, and xAI as evals, cost, and reliability changed. As of 2026-07-13, the workflow's source of truth assigns DeepSeek V4-Pro to Pass 1 propagation, xAI Grok 4.20 to Pass 2 synthesis, and DeepSeek V4-Pro to Pass 3 review. Because Pass 1 and Pass 3 currently share a vendor, this is two-vendor coverage across three passes, not the earlier four-model / three-vendor configuration. Heterogeneity is the principle, not a fixed roster: future workflow changes should preserve cross-vendor coverage while also satisfying the task-independence boundary below.

**BioDesignBench empirical support (2026-05-13, PRIMARY-SOURCE-PENDING):** A bioRxiv preprint (2026.05.06.723381) benchmarking LLM agents on 76 protein-design tasks reports that DeepSeek V3 significantly outperforms GPT-5 and Gemini 2.5 Pro on the benchmark (per abstract; full PDF verification pending). OE's sweep daemon uses DeepSeek V4-Pro (V3's successor) as the Pass 4 cross-vendor peer-reviewer. If the abstract claim holds under full-text verification, the V4-Pro anchor is not only the heterogeneity guard but also the empirically strongest protein-design reasoner among major frontier models — a substantive upgrade to the architectural justification. The paper also introduces a tool-gap vs. science-gap decomposition as a diagnostic framework for agent failures, which is a methodological-upgrade candidate for Pass 3/4 disagreement attribution. See [bio-ai-tools.md §"BioDesignBench"](./bio-ai-tools.md) for full details and verification gate. (source: bio-ai-tools.md; Mechanistic Extrapolation pending primary-source verification)

**Empirical validation and boundary cases.** The guard produced two early documented catches: the DAF SCR1-4 disulfide-count incident (2026-05-06, caught at the pre-commit grep-verify gate — see [`etc/manual-literature-mining.md` §"Pre-commit verification gate"](./manual-literature-mining.md#pre-commit-verification-gate-the-rule-that-catches-errors-before-the-sweep-not-after)) and the Paperclip Km misreport (2026-05-13, caught by DeepSeek V4-Pro's cross-vendor manuscript review after the error had entered the corpus and propagated into the paper draft — see [`etc/paperclip-deep-dive.md` §"2026-05-13 correction"](./paperclip-deep-dive.md)). DAF is the canonical authoring-stage number hallucination; Paperclip is the canonical error that survived single-model generation and review. The 2026-07-13 uricase operating-regime incident below is the complementary failure of the guard itself. The submission-time paper and post-submission working-copy update are at [`papers/cross-vendor-heterogeneity-guard/draft.md`](../../papers/cross-vendor-heterogeneity-guard/draft.md).

**Boundary found 2026-07-13 — vendor diversity is not task independence.** A later whole-corpus audit found that the legacy comp-019 gut-uricase model omitted physiological substrate occupancy and finite residence time even though the relevant luminal-urate concentration and UOX Km were stored in its own input files; it also omitted oxygen access and peroxide burden even though the oxidase stoichiometry and a relevant oxygen-management precedent appeared elsewhere in the corpus. The corpus had been worked on across GPT-5.5, Gemini 2.5 Pro / 3.5 Flash, DeepSeek V4 Pro, and Claude Opus 4.6 / 4.7 / 4.8 (author-reported operational roster; not every model independently audited comp-019). The failure therefore does not show that vendor heterogeneity has no value. It shows that different vendors can remain correlated when they inherit the same prompt, accepted ontology, task role, and source hierarchy. A concrete architecture choice amplified the problem: nineteen long-form comp analyses were compressed into short sweep-visible stubs while their archives and code/input/output artifacts moved below the sweep-excluded experiment boundary for legitimate token-budget reasons. Artifact fetch remained available but optional. The facts were present; no pass made them load-bearing. [Comp-044](../gut-lumen-uricase-physiologic-regime-computational.md) invalidated the old capacity conclusion before oxygen penalties were applied, and [comp-045](../uricase-topology-oxygen-peroxide-design-computational.md) then made physiology-matched topology × oxygen × peroxide gout-program Gate 0. The architecture now requires two orthogonal defenses: **cross-vendor heterogeneity** and **epistemic-role heterogeneity**, including an independent constraint-closure pass that re-derives quantitative claims from reaction stoichiometry, operating concentration versus kinetic constants, residence time, compartment/access, coproduct safety, and actual code use of stored inputs. A separate [comp-review daemon](../../scripts/SWEEP-ARCHITECTURE.md#independent-comp-review-daemon-implemented-2026-07-13) now reviews every changed comp artifact in a bounded context and queues only actionable findings. Repeated cross-page or cross-vendor agreement is one inherited claim until a differently framed pass re-derives it. Full public incident record: [`operations/notable-moments.md` 2026-07-13](../../operations/notable-moments.md); post-submission paper boundary case: [`papers/cross-vendor-heterogeneity-guard/draft.md` §5.7](../../papers/cross-vendor-heterogeneity-guard/draft.md). (source: comp-019/044/045 implementation audit; Mechanistic Extrapolation for generalization beyond this case)
...
- Validation (potential collaborators, actively recruiting): Role 1 (Gut Microbiome / Gnotobiotic Core), Role 2 (Pharma Translation / Regulatory)
