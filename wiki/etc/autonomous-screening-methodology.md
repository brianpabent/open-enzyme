...
**Second instance — pLDDT-as-accessibility proxy (added 2026-05-30, comp-034 rosetta_concordance).** A different cheap proxy, same failure mode. The shared protease-stability model ([`lib/protease_stability.py`](experiments/lib/protease_stability.py) `classify_accessibility`) uses **pLDDT as a proxy for solvent accessibility** (high pLDDT → assumed buried → 0.1× cleavage risk). But pLDDT is a *confidence* score, not burial: a confidently-predicted, solvent-**exposed** helix scores 90+ and gets mis-classified "buried." On the lactoferrin inter-lobe linker (pLDDT 93–98) this under-counted real cleavage risk ~10× — real SASA (PyRosetta SasaCalc, Tien 2013 relative) showed **8 of 11 residues exposed, 0 buried.** The fix (now demonstrated in [`comp-034/rosetta_concordance/`](experiments/comp-034-lactoferrin-linker-redesign/rosetta_concordance/README.md)) is real SASA + a secondary-structure conformation gate (proteases need an extended substrate; a helix resists even when exposed). The proxy affects every comp-NNN using that model (comp-001/005/006/012/037) — their cleavage scores systematically under-weight exposed-but-confident regions. Generalizes the comp-022 lesson beyond RNA folding to the *accessibility* axis: **a structural-confidence score is not a structural-property measurement.** When PyRosetta is available (it now is, since 2026-05-29), prefer real SASA over the pLDDT proxy for any load-bearing accessibility judgment.
...
|---|---|---|---|---|---|
| comp-019 | Gut-lumen uricase × ABCG2 flux model | Yes (Monte Carlo n=5000 across genotype × sex × dose) | **No—invalid regime** (multiple reported metrics inherited one Vmax-only capacity error) | Implicit grid; no empirical calibration | **Superseded by comp-044** — evaluation depth did not protect against omission of substrate occupancy and finite residence time |
