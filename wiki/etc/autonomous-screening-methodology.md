...
**COMP-022 caution:** The invalidated artifact enumerated 43,200 uricase cassette labels, then combined heterogeneous proxies that were not calibrated to one measured outcome. The enumeration survives only as historical inventory; every rank, shortlist, winner, and component preference is retired. The remaining construct variables belong in matched experiments, not a rescued composite. See the [evidence boundary](../uricase-cassette-ranking-computational.md).
...
This supports deeper, genuinely multi-metric evaluation. It does not validate an N-of-M vote whose axes are uncalibrated, correlated, or aimed at different outcomes; retired COMP-022 is the cautionary example.
...
| comp-022 | Uricase cassette ranking (v1 + v2) | Historical 43,200-label enumeration only | Heterogeneous proxies; chaperone-load axis used uncalibrated inherited coefficients | N-of-5 gate was not calibrated to one named biological outcome | **Invalidated tombstone.** Enumeration survives only as historical inventory; no rank, shortlist, winner, or component preference survives. |
...
| comp-026 | Multi-cassette induction interference | Historical multi-cassette enumeration | Included a now-invalid COMP-022 top-cluster comparison | Historical orthogonal-promoter filter | **Not current decision authority.** Re-review independently before reuse; no COMP-022-derived candidate survives by inheritance. |
