...
**Known proxy failure — pLDDT is not accessibility.** The shared protease-stability model ([`lib/protease_stability.py`](experiments/lib/protease_stability.py) `classify_accessibility`) used AlphaFold pLDDT as a solvent-accessibility proxy. pLDDT is prediction confidence, not burial, secondary-structure compatibility with cleavage, protease survival, or retained activity. COMP-001 therefore supplies only P1/P1′ sequence mapping plus a structural-confidence prior for *A. flavus* UOX. It cannot support a LOW-risk, survival, or fermentation-performance conclusion. Any UOX shio-koji survival claim requires the direct [§1.10 protease-stability assay](../validation-experiments.md#110-protease-stability-assay-shio-koji); a structure/SASA analysis may refine candidate selection but cannot replace that gate. The same boundary applies to other COMPs that inherited the proxy.
...
|---|---|---|---|---|---|
| comp-019 | Gut-lumen uricase × ABCG2 flux model | Yes (Monte Carlo n=5000 across genotype × sex × dose) | **No—unconditional classification not robust** (multiple reported metrics inherited one Vmax-only capacity assumption) | Implicit grid; no empirical calibration | COMP-019's unconditional flat-dose classification is not robust to COMP-044's tested substrate-occupancy and finite-window diagnostics. COMP-044 supplies no replacement physiology, dose, efficacy, genotype order, production target, topology/chassis, or safety conclusion. |
| comp-022 | Uricase cassette ranking (v1 + v2) | Yes (43,200 enumerated; 195 pass v1; 71 pass v2 N-of-5 ≥ 4) | Multiple computational axes, with shared assumptions and proxy limits | Prespecified N-of-5 gate | Candidate-prioritization result; exact build, expression, localization, physiological-reaction, and safety gates remain empirical. |
