...
### Protease-candidate-to-redesign workflow

The retired comp-005 → comp-034 chain is a cautionary example, not a validated design-to-wet-lab pipeline: an unverified protease filter was reused as candidate-selection authority. A reusable pipeline must bind verified inputs, keep proxy scores distinct from cleavage evidence, and require direct retained-function testing.

**The four-step pattern:**

1. **Candidate-region mapping.** Map motif matches only from claim-level-provenanced specificity data and report whether each rule is verified or merely inherited. AlphaFold pLDDT may describe model confidence; it must not be used as solvent accessibility. Assembly-aware SASA and conformation checks can refine a prior but cannot establish cleavage or survival. Output: regions prioritized for direct fragment mapping and retained-activity testing.

2. **Candidate classification — required connector vs candidate removable spacer.** For each candidate region, ask whether direct structural and functional evidence makes it a required connector or whether a matched truncation remains plausible. The two classes get different test strategies:
   - **Required connector** → redesign-in-place within verified geometric and functional constraints. The lactoferrin inter-lobe connector is a candidate for this class only if a reproducible WT fragment maps there; COMP-034 did not establish the failure.
   - **Candidate removable spacer** → compare a truncation arm with the full construct. Example: the DAF Ser/Thr-rich stalk (aa 286–353) lies outside the SCR1–4 construct and has lower model confidence; [COMP-012](../daf-cd55-scr14-truncated-computational.md) does not establish dispensability, stability, or retained function.
   The choice remains a construct-design hypothesis until the redesigned or truncated protein is measured for expression, processing stability, and retained function.

3. **New gated redesign.** Begin only after direct data define a reproducible failure. Bind verified specificity inputs, exact structural/functional constraints, and a falsifiable output contract before sequence generation. ProteinMPNN, Rosetta, and language-model scores may diversify or filter candidates; none is a cleavage or process-survival measurement.

4. **Matched diversity panel.** Freeze a panel that spans the prespecified design axes without calling a proxy-score leader the biological winner. Compare every variant with WT for intact recovery, mapped fragments, retained function, and structural stability.

**Validation status:** no protease-resistance candidate-ranking workflow has been validated. Section 1.10 first tests WT retained protein, activity, and fragment formation. A new candidate-design COMP begins only if a reproducible failure supplies an empirical design target.

**Retired COMP-034 boundary:** the ProteinMPNN and Rosetta extensions did not validate the inherited protease filter. No sequence from that artifact is nominated for synthesis. Section 1.10 first asks whether WT produces a reproducible linker-associated failure.
...
- When a sequence/structure screen identifies a structured-mandatory linker as a candidate region for direct testing.
- When a region may be removable, include full-length and truncated construct arms; do not treat truncation as the answer before retained function and route-relevant stability are measured.

**Cross-references:** [`lactoferrin.md` §12 item 13](../lactoferrin.md) (candidate engineering question); [`lactoferrin-linker-redesign-computational.md`](../lactoferrin-linker-redesign-computational.md) (comp-034 invalidation and surviving conjecture); [`lactoferrin-protease-stability-computational.md`](../lactoferrin-protease-stability-computational.md) (retired comp-005 proxy); [`daf-cd55-scr14-truncated-computational.md`](../daf-cd55-scr14-truncated-computational.md) (a separate matched truncation hypothesis).
...
**13 of 19 tools active in CPU mode** (was 9; the four `rosetta_*` tools unblocked 2026-05-29 — see PyRosetta row):
...
- **A1 (CPU-mode local install)** — done 2026-05-15. ProteinMPNN is slow on CPU but usable for future gated sequence-diversity work. COMP-034's substitute-sampler and later ProteinMPNN runs are retired because their biological target was unverified; tool availability does not rescue that design contract.
...
- **A5 (PyRosetta unblock)** — done 2026-05-29. The 2026-04-01 UW export-control hold on academic PyRosetta licensing was resolved by UW CoMotion's academic-license update (K. Brede notice). `pyrosetta 2026.3` (cp313, macOS arm64, 1.53 GB wheel) installed credential-free from the RosettaCommons west mirror; `import pyrosetta` + end-to-end `rosetta_score` (ref2015) through `PyRosettaRunner` both verified. The four `rosetta_*` physics-scoring tools can support a future, newly gated variant COMP; neither retired COMP-002 nor exploratory COMP-034 supplies an authorized ΔΔG-to-process-performance conclusion. Commercial-license caveat noted in the tool table.
