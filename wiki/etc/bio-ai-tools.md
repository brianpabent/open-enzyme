...
2. **Audit OE's other comp-NNNs for shallow-eval risk.** Comp-019 is an invalidated counterexample: multiple reported metrics inherited one unsupported capacity assumption, so output count did not provide orthogonal evaluation. Comp-022 is multi-metric but remains a candidate-prioritization result; comp-023, comp-024, comp-025, comp-026, and comp-027 should be audited against three axes — (a) multiple candidates generated? (b) multi-metric evaluation? (c) head-to-head comparison + filtering? Audit table at [`autonomous-screening-methodology.md` §"BioDesignBench evaluation-depth audit"](./autonomous-screening-methodology.md).
