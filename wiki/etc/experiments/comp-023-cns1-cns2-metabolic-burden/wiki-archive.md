...
(Full numeric output: [`experiments/comp-023-cns1-cns2-metabolic-burden/outputs/results.json`](../comp-023-cns1-cns2-metabolic-burden/outputs/results.json). Reproducible by `python3 analyze.py` in the experiment folder.)
...
**Cross-chassis ADA-protection option (added 2026-05-15).** A separate delivery-side path that avoids the additional cassette complexity of engineering pentostatin into koji: **pair engineered-koji cordycepin (this comp-023 path) with whole-fermentate *C. militaris* extract (native pentostatin source per Xia 2017 PMID 29056419) as a co-formulated product.** Engineered koji handles bulk cordycepin production (the metabolically tractable part this analysis green-lights); a small co-formulated dose of cultivated *C. militaris* extract provides ADA-protecting pentostatin from its native co-evolved chemistry. Avoids the additional cassette OR ADA-knockout work that comp-024 / comp-025 would otherwise gate. Cross-chassis kinetics still gated by [comp-025](./computational-experiments.md) (ADA × cns1 substrate competition modeling). Optional wet-lab co-formulation stability test queued at [`validation-experiments.md` §2.7](./validation-experiments.md) (verify cordycepin stability in engineered-koji fermentate co-formulated with dried *C. militaris* extract, ADA-challenge assay readout). Full description at [`medicinal-mushroom-complement-track.md`](./medicinal-mushroom-complement-track.md) §"Combined / synergy candidates."
...
- [experiments/comp-023-cns1-cns2-metabolic-burden/](../comp-023-cns1-cns2-metabolic-burden/): full analysis script, GEM, inputs, and reproducibility provenance
