...
| secretion_pathway_availability | 0.6 | 0.45 | 0.75 | Sec-translocon present. Same caveat as lactoferrin re: disulfide-rich secretion. 8 disulfides in SCR1-4 — less than lactoferrin's 16 but still requires oxidative folding. |
...
4. **CR1 SCR1-4 ranks slightly above lactoferrin** due to fewer disulfide bonds (8 vs 16) and the SCR/sushi/CCP-fold's documented isomerization tolerance.
...
- **Engineered LBP chassis Phase 2 (P2-4)** per [`wiki/engineered-lbp-chassis.md`](../../wiki/engineered-lbp-chassis.md): F. prausnitzii is engineering-naive as of 2026-05; the LBP-chassis Phase 2 investment should prioritize toolkit development BEFORE payload selection.
...
- **comp-007 (food-grade HDAC inhibitor screen) cross-link:** butyrate was the top food-grade HDAC inhibitor with >=167x class-I-over-HDAC6 selectivity (censored lower bound). Continuous gut-luminal butyrate from an engineered F. prausnitzii strain is PROPOSED to address the bioavailability problem at the dose-frequency level (unproven — requires measured colonization density, butyrate titer, epithelial exposure, and the still-unvalidated direct Q141K rescue) — this comp says that's the FIRST plausible engineering campaign to try, not the second or third; it does not establish that butyrate flux solves bioavailability.
