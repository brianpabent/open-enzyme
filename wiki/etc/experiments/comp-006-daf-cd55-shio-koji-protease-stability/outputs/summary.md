...
- **The ectodomain HIGH verdict is a conservative stress-test, not a survival model, and is dependent on NPr pH factor = 1.0.** The input note itself estimates realistic NPr activity at pH 4.5–5.0 as ~0.3–0.5. At NPr pH factor 0.5 the NPr max score is ~0.194; at 0.3, ~0.116 — both below the HIGH threshold (0.30). Under realistic NPr activity the ectodomain verdict would shift toward MODERATE/LOW, with the disordered stalk remaining the engineering liability. Read HIGH as 'conservative worst-case flag on the disordered stalk,' not 'predicted significant degradation.'
