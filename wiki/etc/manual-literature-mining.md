...
2. **Tier 0 + Tier 1 frequently resolve or sharply reroute the question.** COMP-016's bounded scan did not identify direct androgen-driven intestinal ABCG2 suppression, and COMP-017 corrected the Hoque magnitude, Liu and Slepnev attributions, and exposure interpretation. That was enough to reject the asserted direct-suppression mechanism at the reviewed-source tier. It did **not** resolve healthy-human intestinal sex stratification because no direct GTEx/HPA values were extracted. Tier 0 can therefore kill an overclaim while leaving a narrower measurement question open; a later bench experiment is justified only for what the public evidence could not decide.
...
1. **Does *Candida utilis* uricase have published exact-construct processing or cassette data?** — informs a future sequence-bound construct comparison; retired COMP-011 supplies no KEX2, mutation-benefit, clinical-sequence, or cassette-risk conclusion. Search: `Candida utilis uricase Aspergillus` + grep exact sequence, signal peptide, junction, processing, secretion.
2. ***Aspergillus oryzae* heterologous expression precedents** for exact single- and dual-protein configurations, particularly solid-state formats. Retired COMP-010 supplies no production-capacity conclusion. Search: `Aspergillus oryzae solid-state heterologous protein` + grep dual cassette, multi-cassette, two genes.
...
4. **GRAS-host complement-regulator expression** — has anyone published heterologous expression of soluble complement regulators (sCR1, Factor H, DAF/CD55) in any GRAS organism? Informs [H05](../hypotheses/H05-daf-scr14-cp0-thesis.md), [§1.25](../validation-experiments.md), and future exact-configuration work; retired COMP-006 and COMP-012 supply no survival result. Search: `complement regulator heterologous expression yeast Aspergillus` + grep specific protein names.
