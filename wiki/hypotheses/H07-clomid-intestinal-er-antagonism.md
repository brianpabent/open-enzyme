...
> **Evidence status:** stub. Assumptions, pre-committed thresholds, kill switches, and failure-mode coverage remain incomplete. The core open question is clomiphene stereoisomer behavior in enterocytes.
>
...
1. **Intestinal ER signaling can induce intestinal ABCG2, but physiological magnitude is unestablished.** Yu 2021 used 100 µM EB in Caco-2 cells, 5–6 orders above physiological serum E2, reported no dose-dependent effect, and used an ovariectomized-plus-replacement mouse model rather than a healthy comparison. Klyushova 2023 found T, E2, and P increased ABCG2 at 1–100 µM through PXR/FXR, also above physiological free-hormone exposure. Evidence: **In Vitro + Mechanistic Extrapolation**; healthy-human sex dimorphism remains unsupported.

2. **Clomiphene acts as an ER antagonist in enterocytes.** No direct measurement of clomiphene's agonist/antagonist behavior in human enterocytes was found. Peripheral agonism would predict the opposite direction, so this remains the open core of H07 and requires enterocyte SERM testing.

3. **Direct AR-mediated repression of intestinal ABCG2 is contradicted, but ER antagonism is not yet established as the alternative.** Klyushova 2023 found T induced rather than suppressed ABCG2 in Caco-2 cells; Hoque 2020 did not invoke AR; and the renal evidence does not supply the missing intestinal mechanism. The negative half is supported; the proposed ER mechanism remains contingent on sub-claim 2.

4. **Intervention implications remain conditional.** Direct urate-axis modulators do not depend on the contradicted intestinal-AR mechanism. Claims about aromatase inhibitors or DIM depend on whether physiological E2 meaningfully changes intestinal ABCG2, and the enclomiphene-versus-clomiphene urate differential remains open.
...
3. **The renal URAT1 arm may account for the full observed Clomid-associated UA elevation.** If so, the intestinal-ER hypothesis is not load-bearing for that observation. FEUA tracking can separate the renal contribution in an n=1 case.
...
- **GTEx + Human Protein Atlas mining for sex-stratified intestinal ABCG2 expression in healthy humans.** Heavy overlap would weaken population-level corroboration for sub-claim 1; a meaningful difference would bound the possible effect size.
...
- If multiple SERMs show intestinal-ER agonist behavior in Caco-2, sub-claim 2 fails and the gut-lumen intervention hypothesis requires reframing.
...
- [`gut-lumen-sink.md`](../gut-lumen-sink.md) — the gut-lumen mechanism affected by the intestinal-ABCG2 question.
