...
**Pilot in flight (2026-05-16):** comp-034 (inter-lobe linker redesign for shio-koji protease resistance) is the first concrete use case for the newly installed tool stack — exercises ProteinMPNN against the comp-005 cleavage-site outputs and the comp-022 / comp-030 multi-metric concordance pattern. Outputs feed `validation-experiments.md §1.10` if any candidate passes the multi-metric gate.
