...
**Pilot completed (2026-05-16):** [comp-034](./lactoferrin-linker-redesign-computational.md) (inter-lobe linker redesign for shio-koji protease resistance) — the first concrete use case for the newly installed tool stack. **15 of 60 candidates pass N-of-5 ≥ 3 (GREEN). Zero pass STRICT 5-of-5.** Primary wet-lab variant `EEEEPAARRAR` (S353E + V357P, 82% WT identity) passes 4-of-5; cleavage 0.407 → 0.290 (~29% reduction). Secondary: true single-V357P `SEEEPAARRAR` (91% WT identity, 3-of-5). WT linker is a high-pLDDT structured α-helix (AF mean 95.6), not flexible loop — redesign premise empirically grounded by 16 cleavage sites. Outputs feed `validation-experiments.md §1.10` if any candidate passes the multi-metric gate. (Mechanistic Extrapolation; source: lactoferrin-linker-redesign-computational.md)
