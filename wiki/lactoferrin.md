...
Three wet-lab arms span the design-aggressiveness spectrum (per Pass 3 2026-05-17 grep-verification of the comp-034 archive):
...
- **Aggressive — multi-proline + acidic-substitution variants** (e.g., `DEEDPANPQAH`, `EEEQPQEQRHR`, `EEEEPAAPPAP`, 55–73% WT identity). Cleavage drops are substantial — **>43% from WT** for the top aggressive candidates, with `EEEEPAAPPAP` passing 4-of-5. Higher fold-quality risk in trade.

Outputs feed `validation-experiments.md §1.10` linker-variant arm. (Mechanistic Extrapolation; source: lactoferrin-linker-redesign-computational.md)
