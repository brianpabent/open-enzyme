...
**A separate mechanism class from the chokepoint coverage above.** The §4.5 mapping addresses lactoferrin as an NLRP3-cascade modulator. This section addresses lactoferrin as an **upstream substrate-supply lever** for the platform's primary urate-lowering mechanism — distinct from chokepoint coverage and arguably the more strategically important role in the koji endgame strain.

**The mechanism (Speculative — supporting links Animal Model + In Vitro):**

1. The Open Enzyme platform's gut-lumen-sink mechanism depends on the **ABCG2 transporter** moving urate from blood into the intestinal lumen, where engineered uricase can degrade it (see [gut-lumen-sink.md](./gut-lumen-sink.md) and [abcg2-modulators.md](./abcg2-modulators.md)). ABCG2 is the rate-limiting substrate-supply step for the entire platform mechanism in the gout track.
...
13. **RFdiffusion + ProteinMPNN tool-stack integration for variant design** *(added 2026-05-15)*. Lactoferrin variant design — lactoferricin / lactoferrampin peptide-only constructs per #7 above, host-optimized glycosylation variants per #3, or apo-form-stabilized variants per #4 — is a **redesign** problem. Per the BioDesignBench-validated tool stack (Kim & Romero 2026; see [`bio-ai-tools.md` §BioDesignBench](./bio-ai-tools.md)), the canonical computational tools for structure-conditioned sequence redesign are **RFdiffusion** and **ProteinMPNN**, neither of which is currently in OE's stack. Both are available via the **protein-design-mcp** package ([github.com/jasonkim8652/protein-design-mcp](https://github.com/jasonkim8652/protein-design-mcp), MIT licence, Docker images). **Fires when:** any of #3, #4, or #7 above moves from question to scope-able computational experiment, OR when the koji-Lf production gate (#2) returns a folding or stability problem that the current ESM2 + Boltz-2 stack can't resolve. Until then, dormant. **Phase 2 integration tasks (if triggered):** (a) deploy protein-design-mcp as a sibling MCP server; (b) re-run candidate variants through RFdiffusion + ProteinMPNN + AlphaFold3 for multi-method concordance per [`autonomous-screening-methodology.md` §"BioDesignBench evaluation-depth audit"](./autonomous-screening-methodology.md).
