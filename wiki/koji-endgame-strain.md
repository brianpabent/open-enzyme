...
> **Third-cassette slot design rule (added 2026-05-15):** when extending the dual-cassette strain to three cassettes, **the third slot is reserved for *cytosolic* payloads, not secreted ones.** Per [`chaperone-orthogonal-stacking.md` §5.6](./chaperone-orthogonal-stacking.md) (Design escape — cytosolic third cassette), the framework's §5.5 triple-cassette pessimism (0.35–0.65 synergy range) applies to *secreted-only* triples (uricase + Lf + DAF SCR1-4); a cytosolic third cassette (cns1+cns2 cordycepin, carnS, panD, native ergothioneine biosynthesis) imposes **zero PDI/ERO1 load** and doesn't compete for chaperone capacity. **Practical implication for the §1.9-extended endgame design:** the third cassette slot goes to a cytosolic payload (cordycepin per comp-023 GREEN; or carnosine/panD); DAF SCR1-4 routes onto a separate strain or the engineered-LBP peer chassis. Formal feasibility gate: [comp-028](./computational-experiments.md) (two-orthogonal-models requirement: chaperone framework with explicit confidence bounds + iWV1314 FBA with dual-cassette baseline overlaid on the cytosolic third cassette). This rule generalizes — **the practical stacking limit for the endgame strain is the count of *secreted* cassettes, not total cassette count.** A four-cassette strain of uricase + lactoferrin (secreted) + cordycepin + ergothioneine biosynthesis (cytosolic) is, in principle, no harder on chaperone capacity than the dual-cassette baseline, pending comp-028's verdict.
>
...
**comp-023 cordycepin arm (2026-05-14):** The cns1+cns2 cordycepin biosynthesis pathway is metabolic-burden-feasible at the Jeennor 2023 empirical titer (564 mg/L/day). FBA on the iWV1314 GEM predicts <1% carbon-flux consumption, growth penalty +0.02% vs WT, and 100% kojic acid + ergothioneine yield headroom. The FBA breakpoint where cordycepin demand materially competes with growth is ~1,000× the empirical titer. Carnosine + panD third-cassette alternatives also GREEN. Three open follow-up gates remain: ADA competition (comp-025), dynamic FBA validation (comp-023 v2), and multi-cassette induction interference (comp-026). (Mechanistic Extrapolation; source: cordycepin-cassette-burden-computational.md)

**Dual-axis feasibility of the §1.9-extended cordycepin arm (composition added 2026-05-15):** the cordycepin arm has now cleared in silico pre-feasibility on **both burden axes** before any wet-lab commitment — comp-023 FBA GREEN on the metabolic axis (above) AND [`chaperone-orthogonal-stacking.md` §5.6](./chaperone-orthogonal-stacking.md) PDI/ERO1 load = 0 on the chaperone axis (cytosolic pathway; no signal peptide; no disulfide bonds). The two axes compose: PDI load for the triple cassette is dual-cassette baseline (uricase 0 + lactoferrin 24–40) + cytosolic third cassette (cordycepin 0) = 24–40 (unchanged from dual baseline), while metabolic burden is bounded at <1% of carbon flux per comp-023. **Formal three-axis gate (chaperone + FBA + regulatory architecture per comp-026) is [comp-028](./computational-experiments.md), still pending.** Once comp-028 returns GREEN, the §1.9 design can incorporate the cordycepin arm at marginal cost (~$1,500–2,500). The cytosolic-third-cassette pattern is also the platform-level design rule articulated in §3 above. (Mechanistic Extrapolation; sources: cordycepin-cassette-burden-computational.md + chaperone-orthogonal-stacking.md §5.6)
