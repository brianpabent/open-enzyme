...
**One engineered *A. oryzae* koji strain, layering two heterologous expression cassettes (uricase + lactoferrin) on top of two native host metabolites (kojic acid + ergothioneine), covers five NLRP3-pathway chokepoints — three via direct payload engineering (uricase + lactoferrin), two via *indirect pathway modulation* (NF-κB suppression, ROS scavenging) through native fungal metabolites.** Per comp-014 ([`medicinal-mushroom-compound-mapping-computational.md`](./medicinal-mushroom-compound-mapping-computational.md)), zero fungal compounds directly bind NLRP3, ASC, or C5aR1 in curated ChEMBL data; the kojic acid + ergothioneine coverage at CP1a / CP1b operates upstream of the inflammasome rather than at the target. This page formalizes the coverage matrix and the Ward 1995 glucoamylase-KEX2 dual-cassette gate, and positions the endgame relative to the simpler starting strains in [engineered-koji-protocol.md](./engineered-koji-protocol.md). This is not the Phase 0 starting point. After comp-044/045, [validation §1.33](./validation-experiments.md#133-physiological-uox-topology--oxygen--peroxide-factorial) is the upstream gout-program Gate 0; §1.9 is staged as Lf-only, selected UOX-only, then dual-cassette. The one-strain platform converges only after both single-cassette legs are validated.
...
**Footnote — sex-dimorphic intestinal ABCG2 and the CP0-upstream / gut-lumen-sink leg** [REFRAMED 2026-05-07 per [comp-016](./t-abcg2-suppression-evidence-mining-computational.md)]**.** The uricase column delivers CP0 coverage *only* to the extent ABCG2 actually moves urate from blood into the lumen, where the engineered enzyme can reach it. The intestinal compartment IS sex-dimorphic in a urate-relevant way (Hoque 2020 Q140K mouse: male intestinal ABCG2 protein 88% reduced under genetic LoF + severe hyperuricemia; female Q140K protected) — but the comp-016 17-study primary-literature scan found **zero** primary studies demonstrating direct androgen-driven suppression of intestinal ABCG2 in vivo. The better-supported mechanism for the male-female asymptote difference is **estradiol-positive driving the female arm via PI3K/Akt** (Yu 2021 Nutr Metab), with the male asymptote running lower because male physiology *lacks* that female-positive signal — not because androgens actively suppress ABCG2. The asymptote difference at healthy baseline may be near-null (MacLean 2008 found no sex difference in healthy rat intestinal ABCG2 across all four segments) and primarily emerges under disease state or genetic vulnerability — most clearly in Q141K-positive male gout patients. So the male-demographic-asymptote argument retains its directional conclusion (sex-dimorphism is real for urate handling), but the mechanism story softens from "T directly suppresses ABCG2 → structural ceiling" to "absent estradiol-positive signaling in male physiology + Q141K vulnerability under male physiology → modest dose-response shift, magnitude uncertain at healthy baseline." (Animal Model for Hoque 2020 Q141K LoF; Animal Model + In Vitro for Yu 2021 estradiol-PI3K/Akt; Mechanistic Extrapolation for the absent-female-positive-signal framing of the male asymptote.) The rescue path is unchanged: fermentable fiber → colonic butyrate → PPARγ-driven ABCG2 induction, plus Nrf2-axis inducers (sulforaphane, indole-3-carbinol), plus HDAC-inhibitor Q141K trafficking rescue (orthogonal to the androgen question — unaffected by the comp-016 reframing). See [abcg2-modulators.md](./abcg2-modulators.md) for the full rescue stack, the Q141K-specific dual-mechanism butyrate lever (§6 — preserved unchanged), and [cross-validation.md](./cross-validation.md) Claim 1 for the platform-level framing. The endgame strain's coverage at CP0-upstream is therefore best read as "uricase + adjunct gate-opening stack," not "uricase alone."
...
**Cross-track URAT1 redundancy (added 2026-05-08).** The same URAT1-downregulation countermeasure logic that motivates carnosine in the koji track is independently provided by **cordycepin** in the medicinal-mushroom track (PMID 29422889 URAT1 mRNA reduction; SUA 337→203 µmol/L) and by **astilbin** in the TCM track ([comp-013](./tcm-gout-compound-triage-computational.md), URAT1 expression downregulation in murine PO model). All three target the same chokepoint by the same mechanism class via independent compound classes — emergent platform redundancy, not designed convergence. See [`androgen-urate-axis.md` §"Relevance to the Open Enzyme platform"](./androgen-urate-axis.md) for the cross-track framing. **Engineering nuance:** carnosine's third-cassette feasibility is titer-gated (the §15 wet-lab experiment), NOT chaperone-load-limited — carnosine biosynthesis is cytosolic (CarnS + panD, zero disulfides, zero secretion-pathway burden), so the [chaperone-orthogonal stacking framework](./chaperone-orthogonal-stacking.md) actually FAVORS adding it to the dual-cassette endgame (predicted weighted synergy ≥0.85). The disulfide-load concern that gates triple-cassette stacking applies to **DAF SCR1-4** (separate-strain routing per §1.25), NOT carnosine.
...
> **Third-cassette slot design rule (added 2026-05-15; revised 2026-05-16 — cordycepin removed as candidate):** when extending the dual-cassette strain to three cassettes, **the third slot is reserved for *cytosolic* payloads, not secreted ones.** Per [`chaperone-orthogonal-stacking.md` §5.6](./chaperone-orthogonal-stacking.md) (Design escape — cytosolic third cassette), the framework's §5.5 triple-cassette pessimism (0.35–0.65 synergy range) applies to *secreted-only* triples (uricase + Lf + DAF SCR1-4); a cytosolic third cassette (carnS, panD, native ergothioneine biosynthesis) imposes **zero PDI/ERO1 load** and doesn't compete for chaperone capacity. **Active third-cassette candidates (post-2026-05-16):** carnosine (carnS + panD per §2.5) for the androgen-dominant URAT1 axis, OR native ergothioneine biosynthesis for the CP1b ROS-scavenging arm. **Cordycepin (cns1+cns2) was evaluated and deprioritized** — see §3.5 "Cordycepin third-cassette slot — evaluated and deprioritized" below. The third-cassette slot design *rule* still holds; the *candidate list* shrank. **The platform-level principle stands:** the practical stacking limit for the endgame strain is the count of *secreted* cassettes, not total cassette count.

> **DAF SCR1-4 routing:** DAF SCR1-4 routes onto a separate strain (per §1.25 wet-lab gate) or the engineered-LBP peer chassis; not as a third cassette in the endgame strain. The chaperone-load math for a uricase + Lf + DAF SCR1-4 secreted triple is the §5.5 0.35–0.65 synergy range that the design rule above is built to avoid.
>
> **Named uncertainty on the 0.35–0.65 prediction range** (added 2026-05-15): the lower bound is driven by Lf's transferrin-lobe α coefficient = 2.5 (Notari 2023 in vitro oxidative refolding from a fully denatured state). In vivo ER-assisted co-translational folding with PDI acting on the nascent chain may be substantially faster, collapsing α toward ~1.0 and shifting the prediction upward into "augmentation-feasible." See [`chaperone-orthogonal-stacking.md` §8 item 6](./chaperone-orthogonal-stacking.md) for the framework's own caveat and [§1.9 Lf-alone arm](./validation-experiments.md) (paired with §1.25 DAF SCR1-4 under harmonized NSlD-ΔP10 / solid-state / matching-promoter conditions per §3.5.4) as the designed in-vivo-α resolver. Until the calibration set returns, treat the prediction range as the *bound on confidence,* not as the *point estimate to design against.*
...
**comp-022 v2 gene-synthesis-time refinements (2026-05-14; reframed 2026-07-13):** The ClockBase-style ranking of 43,200 uricase cassette candidates identified three useful refinements inside the koji direct-secretion design space. They should be incorporated only if §1.33 promotes a compatible secreted-koji topology; comp-022 did not compare secretion with intracellular, displayed, or bacterial alternatives.
...
### 3.5 Cordycepin third-cassette slot — evaluated and deprioritized (2026-05-16)

**Verdict: removed from active cassette stack.** Strategic call during 2026-05-15 sweep walkthrough Item 7. The cns1+cns2 cordycepin pathway cleared the metabolic-burden gate cleanly, but the koji-cordycepin engineering effort was deprioritized for three reasons:

**1. No novel chokepoint coverage.** Cordycepin's chokepoint targets (URAT1 modulation per PMID 29422889; AMPK / mitochondrial NLRP3-priming dampening) are already covered by the [`medicinal-mushroom-complement-track`](./medicinal-mushroom-complement-track.md) via cultivation of *Cordyceps militaris* fruit bodies, which natively co-produce cordycepin with pentostatin at the co-evolved ratio. The endgame koji strain doesn't expand chokepoint coverage by adding cordycepin — it duplicates coverage available via a peer track at a different chassis.
...
**3. "Endgame strain = full coverage + simple" principle excludes it.** The endgame strain criterion is: cassettes that deliver *novel* chokepoint coverage koji is uniquely positioned to provide (food-grade chassis, secreted-protein production, home-fermentation accessibility for proteins not otherwise easily obtained). Uricase and lactoferrin meet this criterion (no commercial supplement delivers a therapeutic uricase or oral lactoferrin dose with koji's protective matrix). Cordycepin does not — it's a $20–60/month supplement-shelf nutraceutical with native ADA-protection from cultivated *Cordyceps* extract.

**Active koji endgame strain cassette stack (post-2026-05-16):** **uricase + lactoferrin** (the secreted-protein dual) plus an optional cytosolic third cassette (carnosine via carnS+panD per §2.5, OR native ergothioneine biosynthesis) where the chaperone-load math supports it. **DAF SCR1-4 remains a separate-strain or LBP-peer-chassis target** per §1.25, not a third cassette in the endgame strain.
...
**What got dropped:** comp-025 (ADA competition for cns1 substrate pool), comp-026 (multi-cassette induction interference for the uricase + Lf + cns1+cns2 design specifically), comp-023 v2 (dynamic FBA validation of the cordycepin scenario) — all marked Deprioritized in [`computational-experiments.md`](./computational-experiments.md). comp-028 (triple-cassette endgame strain feasibility) was reframed: the third-cassette slot was always design-space for cytosolic payloads in general, not cordycepin specifically.

**Active cordycepin route in the platform:** cultivation per [`medicinal-mushroom-complement-track.md`](./medicinal-mushroom-complement-track.md). Users who want cordycepin add a commercial *Cordyceps* extract supplement; engineered koji is not the delivery vehicle.
...
The starting strain is a Year 1 engineering target with decades of precedent. The endgame strain is a Year 2–3 target behind two distinct questions: first, whether a koji-compatible UOX topology clears the physiological regime (§1.33/§1.9B); second, whether that winner coexists with lactoferrin (§1.9C). Treating starting and endgame strains as the same timeline or risk level is a calibration error.
...
**Where the LBP track is mechanistically clean for a population the koji track is structurally limited for:** the male-demographic dose-response shift noted in §1's coverage matrix footnote (sex-dimorphic intestinal ABCG2 — REFRAMED 2026-05-07 per [comp-016](./t-abcg2-suppression-evidence-mining-computational.md): the asymptote difference is driven by absent estradiol-positive PI3K/Akt drive on the male side rather than active androgen-driven suppression) compounds with the Q141K variant subgroup (~10% of gout patients in European-ancestry cohorts; ~30–50% in East Asian gout cohorts; broken ABCG2 trafficking under male physiology per Hoque 2020). For Q141K carriers specifically, butyrate is the cleanest known genotype-agnostic intervention — it both induces wild-type ABCG2 (PPARγ pathway) and rescues Q141K trafficking (class-I HDAC inhibition pathway). **The Q141K rescue mechanism is orthogonal to the androgen-axis question and unaffected by the comp-016 reframing.** But oral butyrate is rapidly absorbed in the small intestine and does not reach the colonic crypt where it needs to act. **A colonically-resident engineered *F. prausnitzii* solves this delivery problem in a way the koji chassis cannot** — koji delivers protein payloads to the gut lumen, but it cannot continuously produce a small-molecule metabolite at the colonic epithelium.
...
1. **Does §1.33 identify a koji-compatible UOX topology, and can that winner then coexist with Ward-architecture lactoferrin?** This is now a two-decision sequence: §1.33/§1.9B establish the UOX leg; §1.9C establishes cassette coexistence. Comp-010 found no blocking sequence-level issue in the provisional direct-secretion pair, but it did not establish physiological UOX sufficiency or select topology. Full §1.9 path: 8–12 weeks and $5,265–8,065 once the UOX input is available.
   ...
   **comp-011 update (2026-05-05; sequencing corrected 2026-07-13):** *C. utilis* remains the MODERATE-risk alternative to *A. flavus*. Retain both sequence candidates, but nest their head-to-head inside the §1.33-selected topology rather than ordering both as unconditional direct-secretion §1.9 cassettes.

   **Chaperone-orthogonal stacking framework (2026-05-05):** The endgame strain's "five molecules" thesis is biochemically defensible via the chaperone-orthogonal stacking framework ([chaperone-orthogonal-stacking.md](./chaperone-orthogonal-stacking.md)). The four payloads partition across non-overlapping ER chaperone load classes: uricase (BiP-transit only, 0 disulfides), lactoferrin (PDI-heavy, 16 disulfides per Notari 2023 PMC10465537; architecture-adjusted effective PDI load 24–40 per [chaperone-orthogonal-stacking.md §3.5](./chaperone-orthogonal-stacking.md)), carnosine (cytosolic, bypasses secretion), native digestives (light, well-adapted). Predicted weighted synergy ≥0.85 — approximately Huynh 2020 adalimumab equivalent ER burden. This retroactively explains why the endgame strain design is not greedy: total chaperone burden ≈ one PDI-heavy mammalian glycoprotein, not five independent loads. (Mechanistic Extrapolation; source: chaperone-orthogonal-stacking.md)
