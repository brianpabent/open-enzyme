...
This matrix tests the **delivery** search space for path-dependent narrowing. Some live-organism configurations naturally enter through an enteral route, but purified, encapsulated, killed-cell, and secreted-payload formats can create different route questions. The 503A/503B compounding-pharmacy track ([`compounding-pharmacy-track.md`](./compounding-pharmacy-track.md)) opens the small-molecule formulation surface, although its current emphasis is oral repurposing. The peptide layer ([`bpc-157.md`](./bpc-157.md), [`peptide-gout-addendum.md`](./peptide-gout-addendum.md)) already treats route as a first-class variable.

Route must be chosen for the exact product configuration. A production chassis may supply more than one downstream format, but portability across routes is an engineering hypothesis until purification, formulation, exposure, and safety are demonstrated.
...
| Recombinant proteins / enzymes (uricase, lactoferrin, catalase, DAF/CD55 ectodomain) | 🔬 ([engineered koji uricase](./engineered-koji-protocol.md), [engineered yeast uricase](./engineered-yeast-uricase-proposal.md), [DAF SCR1-4 candidate](./daf-cd55-scr14-truncated-computational.md); intracellular topology offers a co-localization hypothesis, but reaction-site UOX, catalase, oxygen, substrate, and H₂O₂ remain unmeasured) | 🔬 (purified oral uricase precedent: [ALLN-346](./uricase.md); formulation-specific survival and peroxide control required) | ⚪ (no established buccal absorption or locally acting formulation in this corpus) | 🟡 (distal-colon depot of free or encapsulated uricase — bypasses gastric proteolysis, places the formulation near an intestinal urate-excretion compartment — unexplored for OE) |
| Live engineered organisms (yeast, koji, E. coli Nissle, LBPs) | ✅ ([engineered koji](./engineered-koji-protocol.md), [PULSE-style Nissle](./engineered-yeast-uricase-proposal.md), [engineered LBP chassis](./engineered-lbp-chassis.md)) | — (the organism is the delivery vehicle; formulation, survival, containment, and release still matter) | ⚪ (no established sublingual or buccal path in this corpus) | 🟡 (rectal LBP instillation could test direct distal-colon delivery without oral transit; precedent and exact organism-specific feasibility require verification) |
...
| Recombinant proteins / enzymes | 🟡 (SC UOX has unresolved substrate exchange, peroxide, tissue-safety, persistence, and immunogenicity constraints; see §"Why SC uricase remains unresolved" below) | ✅ ([rasburicase](./uricase.md), [pegloticase](./gout-clinical-pipeline.md), [SEL-212 / pegadricase + ImmTOR](./uricase.md) — clinically established; product-specific efficacy and immune handling still apply) | ⚪ (no established IM UOX path in this corpus) | ⚪ (no established intranasal UOX absorption or local-action path in this corpus) |
| Live engineered organisms | ⚪ (no parenteral safety architecture established in this corpus) | ⚪ | ⚪ | 🟡 (nasal probiotics for upper-airway are a research class; gout relevance is unclear) |
| Engineered NPs (PLGA, LNPs, liposomes, exosomes) | 🟡 (SC PLGA depot for slow-release antigen/payload — research class; no ImmTOR portability inference) | ✅ ([SEL-212 pegadricase + ImmTOR](./uricase.md) — PEG-uricase + PLGA-rapamycin tolerogenic NP IV co-administration; [LNP-mRNA standard for systemic mRNA](./modality-chokepoint-matrix.md)) | 🔬 (vaccine standard — IM mRNA-LNP for COVID precedent) | 🔬 (intranasal LNP research class — flu vaccines, mucosal-immunity programs) |
...
- **SC:** Substrate exchange, peroxide control, tissue safety, persistence, and immunogenicity are unresolved for an exact SC UOX configuration. See §"Why SC uricase remains unresolved" below.
...
- **Rectal:** A distal-colon depot could bypass gastric exposure and place a formulation near an ABCG2-rich intestinal compartment. Local urate flux, residence, reaction products, peroxide control, and systemic effect remain unmeasured. (Mechanistic Extrapolation)
- **Intranasal, inhaled:** No UOX configuration in this corpus demonstrates adequate absorption or local substrate flux together with peroxide safety.
- **Transdermal, sublingual:** No UOX configuration in this corpus demonstrates adequate protein transport or a useful local-action mechanism.

### Live engineered organisms

Live-organism delivery is concentrated in enteral and selected mucosal routes. A parenteral live-organism route would require a safety and containment architecture not established in this corpus.

**Genuinely open cell:** rectal LBP instillation for direct distal-colon delivery. This route could avoid oral transit for an oxygen-sensitive chassis, but colonization, containment, dosing, and organism-specific safety require direct testing. (Mechanistic Extrapolation)
...
- **SC PLGA depot:** Research class for slow-release antigen/payload delivery. ImmTOR's IV use in SEL-212 does not establish that its formulation, dose, immune effect, or safety is portable to SC.
...
[Comp-045](./uricase-topology-oxygen-peroxide-design-computational.md) supplies a blocked candidate topology-comparison layout, and [validation §1.33](./validation-experiments.md#133-physiological-uox-topology--oxygen--peroxide-factorial) owns the measurement. [Comp-035](./intra-articular-uricase-h2o2-reaction-diffusion-computational.md) is a Phase-0 prior for a different compartment; its review explicitly leaves the wet-lab H₂O₂ gate open. Neither computation establishes a generally safe catalase ratio or a preferred route.
...
1. **Substrate exchange is unresolved.** A SC depot and a circulating IV enzyme occupy different transport environments. The relevant urate concentration, replenishment rate, enzyme escape into circulation, and systemic effect must be measured for the exact formulation.
...
2. **Rectal depot of engineered uricase formulations for distal-colon ABCG2 proximity.** A rectal route could place live-organism, purified-enzyme, or encapsulated formulations in the distal colon without gastric transit. Whether that produces useful overlap with secreted urate, sufficient residence, controlled peroxide, and a systemic urate effect is unmeasured. (Mechanistic Extrapolation)
