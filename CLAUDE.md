...
**COMP lifecycle gate:** every new or materially revised comp-NNN requires two context-isolated adversarial reviews: one of code, inputs, provenance, rules, and planned outputs before result-bearing execution; another of the complete artifact and every proposed interpretation after execution. Each review binds to a SHA-256 manifest. Any post-run change to the model, code, inputs, parameters, decision rules, or sensitivity plan returns to the pre-run gate. The completed lifecycle remains valid when the pre-run design equals the post-run design and the current COMP artifact equals the post-run snapshot; later edits to canonical wiki pages do not rewrite history or invalidate the authoring gates, because the current push review independently binds and reviews those evolved pages. Follow [`new-comp-experiment`](./skills/new-comp-experiment/SKILL.md) and [`comp-review-manifest.py`](./scripts/comp-review-manifest.py). Push review is an additional backstop, not a substitute.
