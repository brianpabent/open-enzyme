...
**COMP lifecycle gate:** every new or materially revised comp-NNN requires two context-isolated adversarial reviews: one of code, inputs, provenance, rules, and planned outputs before result-bearing execution; another of the complete artifact and every proposed interpretation after execution. Each review binds to a SHA-256 manifest. Any post-run change to the model, code, inputs, parameters, decision rules, or sensitivity plan returns to the pre-run gate. The completed lifecycle remains valid when the pre-run executable design equals the post-run design and the current COMP artifact equals the post-run snapshot; a non-runnable tombstone may receive a Gate-2-only README correction while its invalidation ledger remains unchanged, and an honestly legacy artifact may use an exact independent post-run binding. Later edits to canonical wiki pages do not rewrite history or invalidate the authoring gates, because the current push review independently binds and reviews those evolved pages. Before spending review tokens, push review removes deterministic quarantines and hash-bound tombstones from eligibility, excludes valid Gate-1-only designs, and blocks changed completed artifacts. Quarantine retains and binds the complete artifact plus imported repository-local decision libraries while blocking execution, propagation, synthesis, and routine review. Final retirement requires a bound independent disposition review, complete surviving-detail and dependency-cascade mapping, Brian’s explicit decision, and a batch of no more than three. Cost-deferred reviews remain explicitly blocked rather than reusing stale receipts. Follow [`new-comp-experiment`](./skills/new-comp-experiment/SKILL.md) and [`comp-review-manifest.py`](./scripts/comp-review-manifest.py). Push review is an additional backstop, not a substitute.
