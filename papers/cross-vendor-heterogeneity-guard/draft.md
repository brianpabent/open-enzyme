...
**Status:** Working draft, sessions 1–8 (2026-05-13). Sessions 1–6 drafted abstract + §1–§9, Methods Appendix, Appendix A, Table 1, Figures 1–2, and the §2 bibliography (12 S2-verified citations at that point). Session 7 ran four parallel cross-vendor review passes via OpenRouter (DeepSeek on §4+§5, Gemini on §3+§6+§7, Claude+DeepSeek on §2) and applied 12 catches; the most consequential was the §5.3 Km factor correction (~7,500× → ~7.5×). Session 8 incorporates external review feedback: adds adjacent-literature citations (Peterson 2024 *Knowledge Collapse*, Wright et al. [14] *Epistemic Diversity*, Rudko & Bonab 2025 *Output Homogenization*, Go et al. 2025 *LiRA*; plus Ouyang et al. [5] *InstructGPT* for §2.4 vendor balance), recasts Figure 2 as a qualitative taxonomy, softens "proof" framing to "case-study evidence," and harmonizes the reference scheme. The manuscript posture is **operational case study with N=1 deployment**, not broad empirical proof; controlled ablations are queued as future work. Submission-ready after Zenodo snapshot (see `submission-checklist.md`).
...
A natural reviewer question at this point: is the cross-vendor architecture doing the work, or is the human reviewing its output doing the work? Both — and the architecture's value depends on that division. The cross-vendor passes narrow the surface a human must read from "the entire corpus" to "the verdict-tagged synthesis-queue items," and widen the perspective by surfacing candidate findings the dominant-vendor pipeline alone would not have flagged; a human triages those findings via the `synthesis/queue/` walkthrough, applying judgment about what to action, what to defer, and what to drop. None of the §5 catches were fully autonomous (§5.1 was sweep + manual UniProt walkthrough; §5.2 was a human-specified rerun; §5.3 was a human-curated ground-truth probe; §5.4 was a machine-surfaced observation that a human read, recognized as load-bearing, and elevated to architectural commitment), and Catch 27 in this manuscript's own audit trail is a human-only catch the cross-vendor review pass missed. The cross-vendor pattern is therefore best understood as a *trigger-and-filter for human attention*, not as a replacement for human judgment — which places it closer to PaperOrchestra (human-in-loop for the science, automation for synthesis-and-drafting) than to AI Scientist v2 (full-loop automation) on the spectrum surveyed in §2.5.
...
**Primary drafter, §2 related work:** Anthropic Claude Opus 4.7 driving the **Ar9av/PaperOrchestra community implementation** of the PaperOrchestra writing pipeline (the Song et al. 2026 paper itself ships no code; Ar9av at github.com/Ar9av/PaperOrchestra is the only usable implementation, released as a skill pack for coding agents). Path B install — skill files cloned to `~/paper-orchestra/` and driven manually from this Claude Code session without permanent symlinks into `~/.claude/skills/`. OE-side inputs: outline, the existing §3–§9 drafts, wiki primary sources as the "rough notes and experimental logs" the pipeline ingests. The pipeline's Literature Review agent verifies cited papers exist via Semantic Scholar; the host LLM (Claude in our case) does all reasoning work, which means §2 is Anthropic-driven through and through. The cross-vendor review of §2 (Session 7 Claude+DeepSeek parallel pass) is the architectural step that compensates for this.
...
**Cross-vendor review run summary:** Session 7 fired four parallel review jobs via OpenRouter (deepseek-on-§4+§5, gemini-on-§3+§6+§7, claude-on-§2, deepseek-on-§2). Total cost ~$0.18, total wall time ~131 seconds (parallel). 20 catches surfaced across the four reviewers; 12 substantive corrections applied to the manuscript, 1 correction propagated to the underlying wiki primary source, 2 reviewer false positives logged for transparency. Raw review outputs preserved verbatim at `paperorchestra-workspace/reviews/`.
