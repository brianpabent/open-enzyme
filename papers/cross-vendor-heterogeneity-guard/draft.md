...
A natural reviewer question at this point: is the cross-vendor architecture doing the work, or is the human reviewing its output doing the work? Both — and the architecture's value depends on that division. The cross-vendor passes narrow the surface a human must read from "the entire corpus" to "the verdict-tagged synthesis-queue items," and widen the perspective by surfacing candidate findings the dominant-vendor pipeline alone would not have flagged; a human triages those findings via the `synthesis/queue/` walkthrough, applying judgment about what to action, what to defer, and what to drop. None of the §5 catches were fully autonomous (§5.1 was sweep + manual UniProt walkthrough; §5.2 was a human-specified rerun; §5.3 was a human-curated ground-truth probe; §5.4 was a machine-surfaced observation that a human read, recognized as load-bearing, and elevated to architectural commitment), and this manuscript's own audit trail (Appendix B) records several human-only catches the cross-vendor review pass missed. The cross-vendor pattern is therefore best understood as a *trigger-and-filter for human attention*, not as a replacement for human judgment — which places it closer to PaperOrchestra (human-in-loop for the science, automation for synthesis-and-drafting) than to AI Scientist v2 (full-loop automation) on the spectrum surveyed in §2.5.

### Implications for autonomous-research systems

A class of recent systems aspires to **end-to-end automated scientific research**: hypothesis generation → experiment design → analysis → manuscript drafting, with humans optionally involved at any step (`abent-family/brian/blog/2026-05-08-grounding-the-ai-scientist-hype/substack.md`). For systems that loop AI synthesis as a load-bearing component (every step builds on the previous step's AI output), the epistemic homogenization risk compounds: any blind spot in the system's prior can propagate across hundreds of synthesized hypotheses before any human notices. The cross-vendor heterogeneity guard described here is one tractable defense against that compounding. Autonomous-research systems that deploy a single vendor across all stages are accumulating the same vendor's prior at every loop iteration; the corpus drift is the system's drift.
...
| Methods Appendix | Anthropic Claude Opus 4.7 (context unique to drafter) | — | **Catch 1 (primary drafter confabulated PaperOrchestra's existence on first ask, caught by user push-back). Preserved verbatim in the Methods Appendix as load-bearing reflexive demonstration.** |
