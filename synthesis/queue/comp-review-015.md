...
10. **Remove the false COMP-007 method attribution.** `analyze.py`, README, `inputs/provenance.md`, `outputs/results.json`, and `outputs/summary.md` credit COMP-007 with achievable-concentration/IC50 thresholds that COMP-007 did not implement. Treat those thresholds as local, unvalidated COMP-015 heuristics or remove them; do not use COMP-007 as their provenance. Because this touches COMP-015 code/output metadata, route the correction through COMP-015's authoring lifecycle and verify that regenerated outputs contain no COMP-007 decision-rule attribution.
