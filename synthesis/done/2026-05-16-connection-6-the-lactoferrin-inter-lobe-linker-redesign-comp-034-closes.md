...
**WALKED 2026-05-19 — Closed (worked example + corrected cleavage-improvement range documented in lactoferrin.md §12; generalizable workflow pattern named in etc/bio-ai-tools.md).**

Actioned:
- ✓ Updated `lactoferrin.md` §12 item 13 with three explicitly-tiered wet-lab arms reflecting Pass 3's grep-verified cleavage numbers: Conservative (single V357P ~24%), Primary (S353E + V357P ~29%), Aggressive (multi-proline / DEEDPANPQAH-class >43% from WT). The "pilot completed" subsection now shows the full design-aggressiveness spectrum rather than just the primary candidate.
- ✓ Added new "Protease-vulnerability-to-redesign workflow" section to `etc/bio-ai-tools.md` (under Protein Design & Engineering). Documents the four-step pattern: (1) vulnerability identification comp-005-style, (2) **vulnerability classification — structural-mandatory vs structural-removable** (the discipline that catches the DAF/Lf conflation failure mode), (3) redesign comp-034-style with 5-metric scoring, (4) wet-lab arm validation-experiments §-style. Documents the when-to-apply and when-to-skip criteria.
- ✓ Cross-references between lactoferrin.md, comp-034 stub, comp-005 page, comp-012 page (DAF counterexample), and the bio-ai-tools.md workflow section now form a coherent template that future secreted-payload candidates can apply.
- ProteinMPNN install caveat documented prominently in the workflow section as a known status (E2 subagent firing to resolve).
