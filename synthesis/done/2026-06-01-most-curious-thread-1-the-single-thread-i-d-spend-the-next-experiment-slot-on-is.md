...
# The single thread I'd spend the next experiment slot on is the multi-user community-fermentation pilot (N=8–12, Q141K + CFH Y402H stratified, single engineered-koji strain + standardized Houttuynia extract + butyrate-emphasis fiber + Tier 2/3 batch QC + event-linked biomarkers).
...
> **Pass 3 review — Confirmed.** `[OVERLAP: EXTENSION]` The Most Curious Thread correctly identifies the multi-user community-fermentation pilot as the highest-leverage single experiment, and the reasoning is consistent with the corpus: H08, H09, and RA #3 are independently documented risk cards; comp-038 documents the Tier 2 butyrate gap; genotype-informed-supplement-workflow and quantification-ladder provide the operational backbone. The Pass 2's personal framing ("this is idiosyncratic — the synthesis daemon's Pass 2 would likely converge on the same pilot... but the explicit linkage of the three riskiest assumptions into one design... is my taste") is honest about what the reviewer added vs. what the corpus already contains. The child experiment framing (Tier 2 butyrate validation §1.31 + Houttuynia THP-1 screen §1.30 as upstream gates) is well-structured. Same caveat as PE #1 applies: execution is gated on strain availability (§1.9), but the protocol design can proceed now at zero marginal cost.
